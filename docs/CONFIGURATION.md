...
asterism uses a dual configuration model.

For local development and validation, asterism also reads environment variables
...
- `openai-compatible`: OpenAI-compatible relay using `Authorization: Bearer <key>` and the configured base URL. asterism uses the relay's `/chat/completions` path through the AI SDK.
- `fal-openrouter`: fal OpenRouter relay using `https://fal.run/openrouter/router/openai/v1` by default and `Authorization: Key <key>`. asterism uses the relay's `/chat/completions` path through the AI SDK.
...
Access. The page shows it by default, but asterism does not authorize, probe,
...
tokens only for task-bound evidence planning. asterism does not send email,
...
The intended production OAuth direction is a desktop installed-app OAuth flow
with system-browser authorization, loopback callback, refresh-token storage in
keychain, access tokens kept short-lived, and continued task-bound connector
ingestion. Gmail OAuth remains out of the default local acceptance gate until it
has explicitly opted-in live validation.
...
explicit button-triggered, read-only Docker availability probe; asterism does
...
set, asterism validates those selected files before the model producer runs
...
When this value is empty, asterism blocks Agent CLI execution instead of guessing
...
asterism defaults Agent CLI runs to `native` mode. In this mode asterism does
...
according to that CLI's installed configuration and permission mode. asterism
...
official CLI. asterism adds task state, Task.md/Task Records, source previews,
GoalPilot guidance, and run contracts as surrounding context, then performs
verification and memory proposal after the CLI returns. asterism should not
rewrite ordinary user turns into a separate product-authored request before
calling the CLI.

When a CLI exposes structured terminal events, asterism asks for that stream
...
asterism falls back to the terminal stdout summary instead of treating the run
as failed.

The Agent API Runtime remains a separate future invocation path. Current
asterism execution work prioritizes the native Agent CLI path.
...
satisfies the shared asterism harness gates.
...
- `native`: default. Respect the official CLI's native read-only capabilities.
  For research-like tasks, asterism may also run an optional OpenAI web-search
...
- `restricted`: use only asterism-provided context. Do not use live web,
...
For local non-live acceptance of the asterism web-research bridge, run
...
Codex native Goal Mode is tracked separately from asterism Task Goal. The
...
closed until the asterism evidence gate proves command shape, progress,
...
closed native-goal passthrough should not be interpreted as asterism task
...
is a non-blocking account-readiness gap rather than an asterism workspace safety
failure.
...
- asterism does not pass local AI SDK `execute` handlers to providers
...
To validate that a real provider tool call can cross the asterism RunService
