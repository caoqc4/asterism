...
| Scheduled/event Agent sweep | Built main-process SchedulerService sweep path with Standing Approval, run-limit, bounded Code Agent trigger, timeline evidence, and no provider/Docker/workspace mutation | `npm run accept:scheduled-event-agent-sweep-smoke` | Covered, local non-live |
