...
| Scheduled/event Agent sweep | Built main-process SchedulerService sweep path with Standing Approval, persisted plus in-sweep run-limit counting, duplicate-candidate blocking, bounded Code Agent trigger, trigger Run evidence contract, run-limit evidence, runtime-start requirement evidence, run-status timeline evidence, persisted sweep summaries, disconnected-port skip evidence, in-flight skip evidence, and no provider/Docker/workspace mutation | `npm run accept:scheduled-event-agent-sweep-smoke` | Covered, local non-live |
...
- Scheduled/event Agent sweep: `accept:agent-local` includes `accept:scheduled-event-agent-sweep-smoke`, which runs against built main-process modules and verifies the scheduler sweep checks duplicate confirmed Standing Approval candidates, starts one bounded Code Agent run through the trigger port with the trigger Run evidence contract, run-limit evidence, and runtime-start requirement evidence, blocks the duplicate through in-sweep daily-limit accounting, records `panel.scheduled_event_agent_triggered` with target task and run-status evidence, persists `lastScheduledEventAgentSweepSummary` for manual, terminal, cron, disconnected-port, and in-flight outcomes, keeps the workspace unchanged, and does not call a provider or start Docker.
