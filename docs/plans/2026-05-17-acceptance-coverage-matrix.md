...
- Scheduled/event Agent sweep: `accept:agent-local` includes `accept:scheduled-event-agent-sweep-smoke`, which runs against built main-process modules and verifies the scheduler sweep checks duplicate confirmed Standing Approval candidates, starts one bounded Code Agent run through the trigger port, blocks the duplicate through in-sweep daily-limit accounting, records `panel.scheduled_event_agent_triggered` with target task evidence, keeps the workspace unchanged, and does not call a provider or start Docker.
