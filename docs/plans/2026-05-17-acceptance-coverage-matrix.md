...
| Scheduled/event Agent sweep | Built main-process SchedulerService sweep path with Standing Approval, persisted plus in-sweep run-limit counting, duplicate-candidate blocking, two-tick cron-soak daily-cap evidence, bounded Code Agent trigger, trigger Run evidence contract, run-limit evidence, runtime-start requirement evidence, run-status timeline evidence, persisted sweep summaries, disconnected-port skip evidence, in-flight skip evidence, trigger-port and task-source `sweep_failed` recovery evidence, timeline-failure started-run evidence, and no provider/Docker/workspace mutation | `npm run accept:scheduled-event-agent-sweep-smoke` | Covered, local non-live |
...
| Runtime-native web/search | Optional Codex/Claude native web/search compatibility evidence | `TASKPLANE_RUN_AGENT_CLI_NATIVE_WEB_SEARCH_SMOKE=true npm run manual:agent-cli-native-web-search-smoke` | Manual only; passed locally on 2026-05-27 with Codex CLI 0.125.0; default skipped |
...
- Scheduled/event Agent sweep: `accept:agent-local` includes `accept:scheduled-event-agent-sweep-smoke`, which runs against built main-process modules and verifies the scheduler sweep checks duplicate confirmed Standing Approval candidates, starts one bounded Code Agent run through the trigger port with the trigger Run evidence contract, run-limit evidence, and runtime-start requirement evidence, blocks the duplicate through in-sweep daily-limit accounting, proves a two-tick cron-soak daily cap blocks the second sweep without a second trigger-port call or timeline event, records `panel.scheduled_event_agent_triggered` with target task and run-status evidence, persists `lastScheduledEventAgentSweepSummary` for manual, terminal, cron, disconnected-port, in-flight, trigger-port failure, timeline failure, and task-source failure outcomes, proves `sweep_failed` recovery for trigger-port and task-source failure paths, proves timeline-failure started-run evidence is preserved, keeps the workspace unchanged, and does not call a provider or start Docker.
- Runtime-native web/search: `manual:agent-cli-native-web-search-smoke` is opt-in and default-skipped. On 2026-05-27, it passed locally with Codex CLI 0.125.0, `auth=ready`, `workspace=unchanged`, `phrase=matched`, `network=called`, and `status=passed`, proving the current Codex invocation shape `codex --search exec ...` still works.
- Agent API execution preflight: `manual:agent-api-execution-preflight-smoke` is opt-in and default-skipped. The default path reports `provider=not-called`, `executionRun=deferred`, `promotionReady=no`, `promotionRequirements=0/11`, `requiredGates=0/9`, and `workspace=unchanged`; the live path sends one minimal provider text request and verifies a disposable workspace stays unchanged. On 2026-05-26, it passed locally with `fal-openrouter / google/gemini-2.5-flash`, `provider=called`, `phrase=matched`, `workspace=unchanged`, and `status=passed`. This is provider-readiness evidence, not a packaged task execution path.
