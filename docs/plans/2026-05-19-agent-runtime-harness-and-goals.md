...
- Model-service fallback: allowed only when the selected AI invocation runtime is unavailable for that phase or when the product explicitly marks the surface as lightweight assistance. Fallback must be visible in product copy and must not be described as Agent API runtime execution.

The first implementation target is not to make every phase use CLI immediately. The target is to remove hidden coupling: every model-facing phase should declare whether it uses selected runtime, model-service fallback, or no AI.
...
- Project decomposition already has draft and confirmation harness boundaries, but draft generation still needs an explicit selected-runtime invocation decision. If it uses model-service fallback, that fallback must be visible and temporary.
...
- Global chat and some lightweight summaries can remain model-service assistance, but they must not be described as Agent API runtime. Agent API is reserved for the future peer selected runtime.
...
2. Mark each call as one of: selected AI invocation runtime, model-service fallback, or no AI.
...
- project decomposition remains a Taskplane harness planner flow, but its model-facing draft generation should be routed through the selected AI invocation runtime or explicitly marked as model-service fallback until Agent CLI planning is wired;
...
- AI Runtime configuration now separates three concepts in copy: first-version Agent CLI invocation with execution as its first completed phase, future peer Agent API Runtime invocation, and model-service configuration for global helper/summaries/lightweight model calls.
...
- CapabilityRegistry and ConfigurationSafetyReport now include `agent_api.runtime` as a disabled peer AI invocation runtime, so diagnostics and context manifests see Agent API as a real planned runtime rather than conflating it with model-service configuration.
...
This pass checks the corrected product model against the actual code. The product runtime/harness owns the task flow. Agent CLI and future Agent API are peer AI invocation runtimes selected by the user. The model-service configuration is still a useful provider-backed mechanism for lightweight assistance and some legacy drafts, but it must not be described as the Agent API runtime.

| Runtime phase | Current implementation | Current invocation layer | Evaluation | Next action |
| --- | --- | --- | --- | --- |
| Global chat / global helper | `RightPanel.send` falls through to `window.api.chatWithAI`; `ipcMain.handle('ai:chat')` calls `generateText`. | Model-service assistance. | Acceptable as a lightweight global helper, as long as copy does not imply the selected Agent CLI/API runtime is executing. | Keep as model-service assistance or add an explicit `global_assistant` phase in a future invocation contract. |
...
| Task-bound chat when selected runtime is unavailable | `RightPanel.send` falls back to `window.api.chatWithAI`. | Model-service assistance fallback. | Product copy is now clearer, but the implementation still has no durable fallback reason attached to the conversation/run evidence. | If this becomes user-visible as task execution, record the fallback boundary. Otherwise keep it read-only assistance. |
...
| Task type review | `RightPanel.buildTaskTypeReviewPrompt` inserts a free-text prompt; renderer-side `inferTaskTypeProfile` is rule-based and used for display/capture defaults. | Mixed: local rule inference plus model-service chat when the user asks. | Gap. This is a real runtime phase in the product flow, but today it is not structured, not confirmed, and not routed through a selected AI invocation contract. | Make this the first adapter-neutral cleanup target: add a typed task-type-review proposal, user confirmation, and selected-runtime/fallback metadata. |
| Project decomposition draft | `TasksPage.generateProjectDecomposition` calls `window.api.decomposeProject`; `ipcMain.handle('ai:decomposeProject')` calls `generateText` and validates drafts with `evaluateRuntimeSubtaskDraft`. | Model-service planning. | Mostly aligned at the harness level, but not aligned with the selected-runtime model. It is a runtime phase that should eventually be invocable through Agent CLI or Agent API. | Introduce a `decomposition_draft` invocation contract after task-type review; preserve existing validation and confirmation gates. |
...
| Decision draft | `DecisionService.draft` and process-template selectors use provider-backed structured generation when available, then write through Decision boundaries. | Model-service drafting inside the product harness. | Acceptable as legacy/model-service drafting, but should be named as a runtime phase if selected Runtime becomes responsible for all AI calls. | Inventory as `decision_draft`; do not block Agent CLI alpha on converting it. |
...
The first-version Agent CLI path is compatible with the existing task runtime flow because the harness already owns goal state, context assembly, decomposition confirmation, verification, memory proposal, handoff, and clearance. The remaining mismatch is not that Agent CLI "only executes"; it is that several AI-facing harness phases still call the model service directly instead of going through a selected AI invocation contract. The practical next step is to introduce that contract gradually, starting with task type review because it is currently the weakest link and is already visible in the user's classification issue.

Suggested implementation order:

1. Add shared phase names and request/result types for selected AI invocations, without changing behavior.
2. Convert task type review into a structured, user-confirmed proposal that records invocation layer and fallback reason.
