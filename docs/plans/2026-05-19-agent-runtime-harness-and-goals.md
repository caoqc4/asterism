...
| Decision draft | `DecisionService.draft` and process-template selectors use provider-backed structured generation when available, then write through Decision boundaries. | API runtime path inside the product harness. | Should be named as a runtime phase because selected Runtime is responsible for all AI calls. | Inventory as `decision_draft`; do not block Agent CLI alpha on converting it. |
