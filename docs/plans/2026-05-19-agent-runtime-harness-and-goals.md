...
| Code Agent model producer | `CodeAgentRunService` can retain context and call `prepareCodeAgentModelProducerRuntime` behind explicit provider-call and feature flags, but model-producer mode now blocks selected CLI runtimes before resolving API config. | Future Agent API-like execution path, gated and API-only for provider calls. | Useful compatibility evidence. It already shares context-manifest behavior with Agent CLI, but it is not yet the user-selected Agent API Runtime and cannot act as a hidden CLI fallback. | Reconcile under the future Agent API adapter rather than treating it as a helper. |
...
AI Runtime selection now reflects that same split in the renderer. Agent API Runtime is selectable when a provider is configured and is labeled as partial: provider-backed question, decomposition, decision draft, and scheduled-brief phases can use it, while full task execution run remains in development. If no provider is configured, the API runtime row stays blocked and points the user to Provider configuration rather than acting as a hidden fallback.
