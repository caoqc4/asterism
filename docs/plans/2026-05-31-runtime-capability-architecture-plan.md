...
| Scheduler / Loop Carrier | Scheduled/event Agent triggers now record selected-scheme scheduler-loop gateway evidence. Selected Agent API scheduler paths stay deferred (`schedulerLoopApiDeferred=true`), selected CLI paths can prove CLI-first support, and the compatibility trigger port cannot silently satisfy API scheduler execution. | aligned | `src/main/scheduler/scheduler-service.ts`; `src/shared/agent-orchestration.ts`; `src/shared/product-feature-impact-audit.ts` | Preserve the selected-scheme gateway evidence and keep API scheduler promotion behind scheduler-specific gates. |
...
| Provider capability probe | Provider and tool capability probes are evidence, not execution readiness. Agent API provider readiness requires selected runtime identity, configured provider identity, provider-owned metadata, explicit tool declarations, no startup probe, and matching package identity; settings expose `providerToolReadiness`, `providerToolProbeTaskExecutionReadiness=not_evaluated`, and `providerToolProbePromotesExecutionRun=no` without promoting task execution. | aligned | `src/shared/agent-api-provider-tool-readiness.ts`; `src/shared/capability-registry.ts`; `src/shared/configuration-safety-report.ts`; `src/renderer/pages/ModelPage.tsx`; `scripts/agent-api-provider-tool-readiness-smoke.mjs` | Keep `startupProbe=never` for Agent API provider capability summaries and require per-entrypoint promotion evidence before provider-visible execution. |
