...
- hierarchy consistency diagnostics can find old parent/child mismatches through the service/IPC boundary, can produce a non-mutating repair plan, can apply only revalidated safe repairs, and can explain manual-review conflicts before a confirmed resolution surface exists.
...
- legacy local task hierarchy attributes and manual-review hierarchy conflicts still need a confirmed cleanup/resolution surface after the database relationship becomes authoritative.
