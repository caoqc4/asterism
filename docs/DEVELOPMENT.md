...
- `Tasks`
  Primary task workbench organized into a current snapshot, an action desk, and an activity feed. It includes structured task signals, active waiting items, recent artifacts, quick decision/run actions, related activity, and a task-lifecycle timeline that can surface lightweight follow-up actions. The related-activity area now also acts as an entrypoint into the linked `Decision` and `Run` objects themselves, and key task timeline events can now open the related `Decision` or `Run` object directly in addition to offering task follow-up actions.
- `Decisions`
