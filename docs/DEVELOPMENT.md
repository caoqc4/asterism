...
- `Home`
  Prioritized control surface with recommended actions first, then recent artifacts, key source materials, recent lifecycle activity, recent brief snapshots, and key signals such as waiting, risk, and missing-next-step tasks. These home surfaces now act as intent-aware entrypoints into task follow-up work, not just passive status cards. Recent lifecycle activity now supports both returning to the related task with a follow-up draft, directly opening the linked `Decision` or `Run` object for closer review, and triggering lightweight follow-up actions for the clearest recent outcomes. Key source materials now surface task-linked external references directly on the home brief and can return the user to the related task with the matching source-context editor already focused. Home brief generation now also aggregates active-task process-template candidates so the scheduler can optionally shape summaries through the most relevant attached methods.
- `Tasks`
