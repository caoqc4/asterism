...
- `Home`
  ...
  The home layer now also carries hidden cross-task `Priority Lanes` semantics: the control surface classifies tasks into escalation, unblock/decide, continue/review, clarify, or steady lanes, then reuses that same urgency model to sort recommended actions, choose the hero headline/lede, and shape written brief text without adding another top-level UI block. Recommended-action cards now also surface a lightweight lane label so users can see why an item was prioritized without turning the home surface into an explicit lane board.
- `Tasks`
