...
asterism uses layered local verification:
...
Contract/verifier/memory-proposal evidence, asterism-owned `/goal`
...
remaining gap is account/provider readiness rather than asterism workspace
...
`skipReason=opt_in_required`, to make clear that asterism-owned task
...
Codex Goal Mode is version-aware in asterism. Codex CLI `0.133.0+` is modeled
...
work should remain focused on asterism-owned durable goals, verifier evidence,
...
provider can answer one request through asterism Agent API Runtime, while full
