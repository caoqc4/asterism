...
- Agent execution layer Phase 1 has started: Runs now have a persisted step trace spine, current text runs write plan/model/final steps through a `RunOrchestrator`, agent run requests now have a typed working-context/policy contract, and the Runs page shows execution-step summaries.
...
1. Continue agent execution layer Phase 1 into a cleaner orchestrator boundary after the run-step trace spine settles.
