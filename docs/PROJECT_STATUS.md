...
- The first domain-shaped task mutation tools are in the registry but not prompt-exposed: `task.update_next_step` routes through `TaskService.update`, `task.create_completion_criterion` routes through `TaskService.createCompletionCriteria`, `source_context.create` routes through `TaskService.createSourceContext`, all write run-step observations, and normal agent plans still fall back if a model proposes them.
