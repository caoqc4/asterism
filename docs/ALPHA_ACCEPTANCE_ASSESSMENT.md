...
- agent execution tests cover read-only workspace tools, an isolated RunService read-only workspace agent path through persisted run detail, policy-gated workspace plan steps, per-run workspace opt-in, session capability metadata, Runs-page workspace / patch-command capability visibility, prompt guidance that only exposes workspace tools when opted in, normal-run fallback when a model proposes `workspace.write_patch` or registry-only domain mutation tools, confirmation-gated `workspace.write_patch`, workspace-boundary / expected-file rejection, service-routed task next-step / completion-criterion / source-context tools through real SQLite repositories, draft-only Decision proposals, Runs-page patch and command checkpoint summaries, Decision checkpoint consequence wording, and Decision approval resumption through real SQLite repositories
