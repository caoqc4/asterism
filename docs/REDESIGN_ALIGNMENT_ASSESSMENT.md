...
| Right panel | Mostly aligned | Global capture, task context, soft context switching, full-screen mode, history summary, task capture confirmation, and session refresh are present. Manual alpha testing clarified that automatic context clearing and user-initiated new conversation must be separate concepts. | Full archived transcript browsing is still minimal. Context clear safety and new-conversation semantics are now captured in [CONTEXT_CLEAR_AND_NEW_CONVERSATION_DECISION.md](CONTEXT_CLEAR_AND_NEW_CONVERSATION_DECISION.md). |
...
| Session as temporary work memory | Mostly aligned | Right-panel history explains current conversation is temporary, and task welcome seed includes task memory, execution records, key sources, and work habits. Manual testing clarified that "new conversation" should be a new free discussion space, not an implied continuation of the old task discussion. | Full transcript archive browsing is minimal. |
| Session refresh / compression | Needs product refinement | Refresh suggestions trigger on repetition, correction churn, generic replies, or compression threshold. Current preservation is lightweight and explicitly avoids saving full chat text. | Context clearing should become a safety-gated handoff check before clearing, while user-initiated new conversation should archive if needed and then start unbound or explicitly rebound. |
