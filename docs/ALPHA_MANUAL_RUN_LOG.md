...
| Release Readiness | Build/smoke/package readiness | Partial | `npm run build`, `npm run smoke:build`, and `npm run dist:mac:dir` pass. The unpacked app exists at `release/mac-arm64/Taskplane.app` (301M), includes `app.asar`, unpacked native modules for `better-sqlite3` and `keytar`, valid `Info.plist` metadata, and passes `codesign --verify --deep --strict --verbose=2`. The generated app was not launched because running newly generated software needs explicit action-time confirmation. | Medium | Launch the packaged app with isolated userData after confirmation, then defer signed/notarized release work until that runtime check passes. |
