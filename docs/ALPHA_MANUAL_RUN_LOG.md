...
| Completion Loop | Add/satisfy/reopen criteria | Partial | Seeded one satisfied and one open completion criterion in isolated userData after the UI jumped to the criteria display area but the creation controls remained awkward to reach through automation. | Low | Exercise criteria create/satisfy/reopen from UI once long-detail navigation is smoother. |
