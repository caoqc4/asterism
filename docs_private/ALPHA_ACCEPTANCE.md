...
## Unified Tasks Workspace

Use this path for the Tasks workspace, task files, and right-panel conversation
redesign.
...
- Edit an editable task file, attempt to switch away, and confirm the Save /
  Discard / Cancel guard appears and behaves correctly.
- Confirm `Task Records/` exists for the task and that context-refresh or
  phase-closeout records appear there after the corresponding right-panel
  action.
- In the right AI panel, switch context cleanup between automatic check,
  manual confirmation, and reminder-only. Confirm manual confirmation archives
  first, then requires a second confirmation before clearing.
- Start a new conversation from history and confirm it returns to global
  context instead of pretending to continue the previous task.
- Close a task phase from the right panel and confirm follow-up tasks retain a
  source record linking them back to the parent phase closeout.
