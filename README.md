...
- blocker-aware recovery semantics, including a current blocker object in task detail, blocker-driven recommended actions and resume wording, optional linkage from blockers into source materials, blocker lifecycle timeline events, blocked-task key signals on the home surface, lightweight blocker-source entry from those home signals, a conservative home-level `标记已解除` action that can also restore clearly blocker-driven waiting states, blocker `created / resolved` lifecycle activity on the home surface, and lightweight blocker-age cues so the UI can distinguish fresh blockers from stale ones
