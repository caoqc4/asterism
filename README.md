...
- task detail acting as a task work surface, now organized more explicitly around recovery first: a resume card and current working slices stay in the first screen, quick actions and state transitions remain in the action desk, related activity and timeline stay in the activity feed, and full source/process management is intentionally pushed into a separate context-management layer so the top of the page stays focused on resuming and continuing the task
