...
describe('product feature impact audit', () => {
  ...
  it('records scheduler Decision approval-item conversion evidence', () => {
    ...
    expect(evidence).toContain('approvalPlanConfirmationSurface=task_dynamics_scheduler_decision_approval_queue');
  ...
  it('tracks the current native CLI writeback and research progress support without stale gaps', () => {
    ...
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
      .toContain('keeps the reversible proposal card missing when its proposalId, evidenceRunId, parentTaskId, subtask count, subtask title identity, subtask rationale identity, subtask dependency identity, or subtask-title uniqueness does not match the same apply-plan evidence chain');
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
      .toContain('proposal cards from another decomposition run cannot be stitched into the current create-many apply plan');
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
    ...
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
      .toContain('proposalSubtaskTitleEvidenceChain, applyPlanSubtaskTitleEvidenceChain');
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
    ...
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
      .toContain('proposalId, expectedProposalId, proposalIdEvidenceChain, proposalEvidenceRunId, proposalEvidenceRunChain, proposalParentTask, proposalTaskEvidenceChain, proposalSubtaskCount, applyPlanSubtaskCount, proposalSubtaskEvidenceChain, proposalSubtaskTitles, applyPlanSubtaskTitles, proposalRationales, applyPlanRationales');
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
      .toContain('proposalSubtaskUniqueChain, proposalSubtaskIdentityChain');
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
  ...
  it('records provider-native session payload identity gating', () => {
    ...
    expect(capabilities?.evidence.join(' ')).toContain('plus nonempty providerCallIds and provider web/search call tool names from provider_payload evidence');
    expect(capabilities?.evidence.join(' ')).toContain('runtime projection, or hand-shaped proposal without provider-owned web/search tool-call identity evidence cannot cross the provider-native session boundary');
    ...
    expect(capabilities?.evidence.join(' ')).toContain('enabling provider-native tool calls does not imply a live provider-native web/search session without payload-owned providerCallIds and web/search tool-name evidence');
  });
  ...
  it('records provider-native session readiness chips and smoke evidence', () => {
    ...
    expect(evidence).toContain('providerNativeSessionRequirements=2/6');
    expect(evidence).toContain('providerNativeSessionMissingRequirements=provider_payload_identity,normalized_plan_identity,provider_call_ids,provider_web_search_calls');
    ...
    expect(evidence).toContain('providerNativeReadySessionRequirements=6/6');
    expect(evidence).toContain('payload provider identity, normalized plan provider identity, providerCallSource=provider_payload, providerCallIds, and provider web/search call tool names all match the selected runtime provider');
  });
