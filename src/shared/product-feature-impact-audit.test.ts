...
describe('product feature impact audit', () => {
  ...
  it('tracks the current native CLI writeback and research progress support without stale gaps', () => {
    ...
    expect(rightPanel?.evidence.join(' ')).toContain('service-evidence=3/11 requirements and 3/9 gates');
    expect(rightPanel?.evidence.join(' ')).toContain('providerConfigured=ready, configuredProvider=openai, providerStartupProbe=not_called, selectedRuntimeRunEvidenceChain=missing, providerPreflightRunEvidenceChain=missing, and providerPreflightTaskEvidenceChain=ready');
    ...
    expect(capabilities?.evidence.join(' ')).toContain('provider configuration alone, anonymous provider configuration, generic unknown-provider metadata, mismatched provider metadata, mismatched tool-declaration package identity, unrelated provider-owned function tools, generic bare search tools, and generic file_search/database_search declarations stay providerToolReadiness=not_declared');
