...
describe('product feature impact audit', () => {
  ...
  it('tracks the current native CLI writeback and research progress support without stale gaps', () => {
    ...
    expect(decisions?.gaps.join(' ')).toContain('SchedulerService proposal producer');
    expect(decisions?.gaps.join(' ')).toContain('concrete scheduler policies');
    expect(decisions?.nextActions.join(' ')).toContain('SchedulerService.proposeSchedulerDecision');
    expect(decisions?.nextActions.join(' ')).toContain('run/sweep evidence');
    ...
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
      .toContain('proposalId, parentTask, subtaskCount, evidenceRunId');
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
    ...
    expect(taskFiles?.evidence.join(' ')).toContain('patchArtifactId, promotionDecisionId, patchRunId, decisionRunId, preflightRunId, sameRunId, and touchedFileCount identity evidence');
