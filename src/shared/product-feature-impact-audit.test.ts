...
describe('product feature impact audit', () => {
  ...
  it('tracks the current native CLI writeback and research progress support without stale gaps', () => {
    ...
    expect(decisions?.gaps.join(' ')).toContain('authorized proposal timeline event');
    expect(decisions?.gaps.join(' ')).toContain('scheduler-origin Decision drafting remains deferred');
    ...
    expect(decisions?.nextActions.join(' ')).toContain('panel.scheduler_decision_proposed');
