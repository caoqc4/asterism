...
describe('product feature impact audit', () => {
  ...
  it('audits core rule docs for business-line-first readiness without banning task execution language', () => {
    ...
    const agents = BUSINESS_LINE_FIRST_RULE_LAYER_AUDIT.find((check) => check.id === 'agents_business_line_owner');
    expect(agents?.requiredFragments).toContain('owns durable business-line state');
    expect(agents?.requiredFragments).not.toContain('Taskplane owns durable business-line state');
  });
  ...
  it('tracks the current native CLI writeback and research progress support without stale gaps', () => {
    ...
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
      .toContain('reversible proposal card, draft evidenceRunId');
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
  ...
  it('records provider-native session readiness chips and smoke evidence', () => {
    ...
    expect(evidence).toContain('providerNativeSessionRequirements=2/7');
...
function readRuntimeArchitectureCloseoutSources() {
  return {
    ...
    rc_test_plan: rcManualChainPublicEvidence,
  };
}
