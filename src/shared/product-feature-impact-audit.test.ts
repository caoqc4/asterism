...
describe('product feature impact audit', () => {
  ...
  it('tracks the current native CLI writeback and research progress support without stale gaps', () => {
    ...
    expect(rightPanel?.evidence.join(' ')).toContain('providerConfigured=ready, configuredProvider=openai, providerStartupProbe=not_called, providerPreflightRunEvidenceChain=ready, and providerPreflightTaskEvidenceChain=ready');
    ...
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
      .toContain('keeps draft_only_timeline_evidence missing when apply-plan evidenceRunId and timeline evidenceRunId diverge');
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
      .toContain('keeps the reversible proposal card missing when its proposalId, parentTaskId, subtask count, subtask title identity, or subtask-title uniqueness does not match the same apply-plan evidence chain');
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
    ...
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
      .toContain('confirmationBoundary, draftOnlyBeforeConfirmation, runtimeMode, and invocationLayer');
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
    ...
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
      .toContain('Agent API project decomposition drafts now return promotionReadiness');
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
    ...
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
      .toContain('proposalId, expectedProposalId, proposalIdEvidenceChain, proposalParentTask, proposalTaskEvidenceChain, proposalSubtaskCount, applyPlanSubtaskCount, proposalSubtaskEvidenceChain, proposalSubtaskTitles, applyPlanSubtaskTitles, proposalSubtaskUniqueChain, proposalSubtaskIdentityChain');
    expect(PRODUCT_FEATURE_IMPACT_AUDIT.find((item) => item.id === 'task_creation_and_project_decomposition')?.evidence.join(' '))
    ...
    expect(taskFiles?.evidence.join(' ')).toContain('expectedFileCount, expectedFiles, touchedFileCount, touchedFiles, filePathSafetyChain, and touchedFileEvidenceChain');
    ...
    expect(taskFiles?.evidence.join(' ')).toContain('expectedFileCount, expectedFiles, touchedFileCount, filePathSafetyChain, and touchedFileEvidenceChain identity evidence');
    ...
    expect(capabilities?.evidence.join(' ')).toContain('provider configuration alone, anonymous provider configuration, mismatched provider metadata, mismatched tool-declaration package identity, and unrelated provider-owned function tools stay providerToolReadiness=not_declared');
    ...
    expect(capabilities?.evidence.join(' ')).toContain('declaredWebSearchToolCount');
    expect(capabilities?.evidence.join(' ')).toContain('declaredWebSearchTools');
