...
describe('capability registry', () => {
  ...
  it('can promote product surface statuses out of deferred capability rows', () => {
    ...
    expect(registry.find((entry) => entry.id === 'agent_api.runtime')).toMatchObject({
      ...
      access: 'mutating',
      requiredGate: 'runtime_pre_step',
      summary: expect.stringContaining('executionKind=api / status=partial / supportedPhases=chat,decomposition,decision,scheduled_brief / executionRun=deferred / executionRunKeyGates=runtime_context_assembly,context_readiness,task_memory_coverage,task_memory_guidance,pre_step,subtask_start,post_step / providerToolReadiness=not_declared / startupProbe=never / selected=true / provider=configured'),
    });
  ...
  it('marks selected Agent API Runtime as available for supported provider-backed phases', () => {
    ...
    expect(registry.find((entry) => entry.id === 'agent_api.runtime')).toMatchObject({
      ...
      configured: true,
      missingReason: null,
      summary: 'executionKind=api / status=partial / supportedPhases=chat,decomposition,decision,scheduled_brief / executionRun=deferred / executionRunKeyGates=runtime_context_assembly,context_readiness,task_memory_coverage,task_memory_guidance,pre_step,subtask_start,post_step / providerToolReadiness=not_declared / startupProbe=never / selected=true / provider=configured',
    });
