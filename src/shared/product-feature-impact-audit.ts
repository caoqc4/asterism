...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Scheduled/event Agent sweep results now expose skipReason, startedRunIds, blockedReasons, blockedTaskSummaries, runFailureReasons, runtimeStartMissingRequirements, terminalRunEvidenceMissingRunIds, triggerRunEvidenceRequired, and triggerRunEvidenceStatus at the top level, so background automation summaries preserve completed/skipped cause, which run started, terminal failure reasons, which task was blocked by which reason, which runtime-start requirements remain missing, which started runs still lack terminal Run evidence, which trigger evidence is required, and whether trigger evidence is still waiting for terminal review without parsing nested summaries.',
