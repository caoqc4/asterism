...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'AgentAutomationReadiness now returns satisfied and missing requirement lists plus automationReady, requirements=x/9, missingRequirements=..., and automationMissingRequirements=... summary evidence, so mature scheduled/event tasks can show that only scheduled_event_entrypoint remains missing.',
      ...
      'SchedulerService.triggerScheduledEventAgentRun now provides a narrow main-side trigger-service connection: it requires an injected Code Agent trigger port, reuses Standing Approval and persisted same-day run-limit checks, starts only ready plans with schedulerTriggerServiceConnected=true, and emits a bounded model-producer Code Agent run request with operatorConfirmed=true plus target task id, task-memory guidance including first open completion criterion and first source title, Standing Approval policy id and scope, runtime-start requirement evidence, run-limit evidence, post-step terminal-evidence guidance, and explicit workspaceWriteAllowed=false proposal-only boundary.',
      ...
      'RuntimeEventRecord now formats panel.scheduled_event_agent_triggered payloads into readable Task Dynamics detail with run id, target task id, trigger plan summary, run status, failure reason when present, terminal Run evidence status, trigger evidence status, required trigger evidence items, runtime-start gate status, localized trigger kind labels for 自动巡检 and 手动启动, Standing Approval policy id, run-limit usage, and workspace proposal-mode write boundary.',
      ...
      'Scheduled/event Agent sweep results now expose skipReason, checkedTaskIds, startedRunIds, blockedReasons, blockedTaskSummaries, runFailureReasons, runtimeStartMissingRequirements, terminalRunEvidenceMissingRunIds, triggerRunEvidenceRequired, and triggerRunEvidenceStatus at the top level, so background automation summaries preserve completed/skipped cause, which tasks were evaluated, which run started, terminal failure reasons, which task was blocked by which reason, which runtime-start requirements remain missing, which started runs still lack terminal Run evidence, which trigger evidence is required, and whether trigger evidence is still waiting for terminal review without parsing nested summaries.',
