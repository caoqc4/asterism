...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'RuntimeEventRecord now formats panel.scheduled_event_agent_triggered payloads into readable Task Dynamics detail with run id, run status, terminal Run evidence status, trigger evidence status, trigger kind, and Standing Approval policy id.',
      ...
      'Brief now surfaces schedulerStatus.lastScheduledEventAgentSweepAt as an automatic-sweep status chip when the scheduler is enabled, so scheduled/event automation health is visible from the operator home surface instead of only service logs or acceptance output.',
