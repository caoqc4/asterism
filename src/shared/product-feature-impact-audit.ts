...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'The subtask create-many apply plan readiness smoke now runs buildSubtaskCreateManyWritebackApplyPlan and dispatchTaskplaneWritebackApplyPlan as a read-only build-gated harness with stale-build detection, proving both agent_cli_decomposition and agent_api_decomposition sources produce subtask.create_many plans with parentTaskId, subtaskCount, panel.project_decomposed timeline evidence, confirmationBoundary=operator_confirmed_subtask_create_many, and draftOnlyBeforeConfirmation=true, proving the Agent API apply plan can complete through mocked injected ports with apiDispatchStatus=completed, apiDispatchAction=subtask.create_many, apiDispatchCreatedTaskCount=1, apiDispatchTimelineType=panel.project_decomposed, apiDispatchTimelineChildTaskIds=mock_child_1, and apiDispatchTimelineRecordPath=Task Records/mock-project-decomposition.md, and proving missingConfirmationDispatchStatus=blocked plus missingConfirmationCreateSubtasksCalled=no when the apply plan lacks the operator confirmation boundary, without provider calls, real writeback dispatch, durable subtask creation, or workspace writes.',
