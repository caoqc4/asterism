...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  {
    ...
    evidence: [
      ...
      'Right-panel Agent API execution requests and progress intents now pass the bounded Pilot decision snapshot into RunService and persist it in the Agent API execution promotion readiness step input, so run evidence can tie the operator-facing request to the product control-layer decision instead of relying only on prompt text.',
      ...
      'Post-run Agent API execution promotion readiness now treats source_context.create as a reviewable non-workspace Write Intent: source-context-only runs can satisfy write_intent_extraction with writeIntentActionIdentityChain=ready and reviewed_patch_apply_boundary with noWorkspaceWriteRequired=yes plus patchPromotionStatus=not_required, so research/source capture can promote without requiring a fake patch artifact.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'The scheduler Decision proposal readiness smoke now runs planSchedulerDecisionProposal, planSchedulerDecisionProposalFromEvidence, and the TaskplaneWritebackApprovalItem conversion as a read-only build-gated harness, proving blocked=0/4 requirements, operator-confirmed=4/4 requirements with operatorId, decisionPayload=ready, decisionTitleKey=confirm_scheduler_action, decisionOptionKeys=approve,hold, decisionProposedOutcomeKey=approve, evidenceSourceType=run, evidenceRunId, evidenceSourceIdentityChain=ready, authorizationCount=1, and authorizationEvidenceChain=ready evidence, local-recovery=4/4 requirements with decisionPayload=ready, decisionTitleKey=confirm_scheduler_action, decisionOptionKeys=approve,hold, decisionProposedOutcomeKey=approve, evidenceSourceType=run, evidenceRunId derived from recovered run identity, authorizationCount=1, authorizationEvidenceChain=ready, localRecoveryRunId, localRecoveryTask, and localRecoveryTaskMatched=yes evidence, standing-approval=4/4 requirements with evidenceSourceType=system, evidenceSourceIdentityChain=ready, authorizationCount=1, authorizationEvidenceChain=ready, standingApprovalPolicyId, standingApprovalScopeTask, and standingApprovalScopeMatched=yes evidence, scope-mismatch=3/4 requirements with authorizationCount=0 and authorizationEvidenceChain=missing, service-evidence=3/4 requirements with approvalQueueSurface=task_dynamics plus decisionPayload=ready, decisionTitleKey=confirm_scheduler_action, decisionOptionKeys=approve,hold, decisionProposedOutcomeKey=approve, evidenceSourceType=run, evidenceRunId, authorizationCount=0, and authorizationEvidenceChain=missing when Standing Approval scope does not match the target task, service-evidence-ready=4/4 requirements with run evidence source identity and operator_confirmation authorization from structured service evidence, and approvalItemDecisionCreateReady=yes with approvalItemKind=scheduler_decision, approvalPlanAction=decision.create, approvalPlanSourceId=run_scheduler_service_ready_smoke, approvalPlanTask=task_scheduler_decision_service_ready_smoke, approvalPlanOptionCount=2, and approvalPlanRecommended=Approve while keeping direct decisionPersistenceAllowed=false, writebackDispatchAllowed=false, and schedulerTriggerAllowed=false without provider calls, direct Decision persistence, direct writeback dispatch, scheduler triggers, or workspace writes.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Runtime patch promotion routing now requires selectedRuntimeContract to carry the same run id and target task id as the reviewed patch artifact, and requires API runtime contracts to carry selectedRuntimeProvider identity plus matching configuredProvider evidence when provided, before selected_runtime_contract can be ready, so future API/runtime patch promotion cannot be promoted from mode/layer/phase metadata alone or stitched configured-provider evidence.',
