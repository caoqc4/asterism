...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  {
    ...
    evidence: [
      ...
      'Agent API execution promotion readiness now binds reviewedPatchApplyBoundary to the Write Intent mode: artifact.propose plus task_file.propose must satisfy an applied reviewed-patch boundary, while source_context.create-only and explicit no-write runs may satisfy noWorkspaceWriteRequired; mismatched patch/no-write evidence leaves reviewed_patch_apply_boundary missing with reviewedPatchBoundaryMode=no_workspace_write_mismatch or patch_apply_mismatch.',
