...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'evaluateAgentApiProviderToolReadinessFromEvidence now derives Agent API provider tool readiness from structured service evidence for selected API Runtime, configured provider identity, provider configuration, no-startup-probe policy, matching provider-owned metadata, and explicit provider-owned web/search/browse tool declarations whose package identity matches the same provider-owned metadata, and now requires providerConfigured=true to carry a configuredProvider identity before satisfying provider_configured while requiring unknown-provider metadata to identify the configured provider by exact owner, package scope, package basename, or package basename prefix rather than a loose substring match, and requiring known OpenAI/Anthropic provider metadata to use provider package scope, exact package basename, or owner-only metadata with no contradictory package instead of third-party package-name prefixes or third-party packages that merely claim a known-provider owner, so provider configuration alone, anonymous provider configuration, generic unknown-provider metadata, mismatched provider metadata, known-provider third-party prefix package names, known-provider owner metadata from third-party package identities, loose provider-name substring package matches, mismatched tool-declaration package identity, unrelated provider-owned function tools, generic bare search tools, and generic file_search/database_search declarations stay providerToolReadiness=not_declared.',
