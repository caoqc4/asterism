...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Agent API decomposition promotion readiness now includes proposal/apply-plan summary, acceptance-criteria, and dependency evidence chains in the reversible proposal card identity check, so a child-task draft cannot be promoted when only titles match but the operator-visible summary, acceptance criteria, or dependency diverges from the subtask.create_many apply plan.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Scheduler/background Decision proposal plans now return satisfied and missing requirement lists plus proposalReady, requirements=x/4, proposalRequirements=x/4, proposalSatisfiedRequirements=..., approvalQueueSurface, decisionPayload, decisionTitle, decisionRationale, decisionOptions, decisionOptionIdentity, decisionProposedOutcome, decisionProposedOutcomeMatchesOption, localRecoveryRunId, localRecoveryTask, localRecoveryCompleted, localRecoveryTaskMatched, operatorId, standingApprovalPolicyId, standingApprovalScopeTask, standingApprovalActive, standingApprovalScopeMatched, missingRequirements=..., and proposalMissingRequirements=... summary evidence without opening Decision persistence, writeback dispatch, or scheduler triggers.',
