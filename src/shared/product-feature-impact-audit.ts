...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Scheduled/event Agent sweep results now expose skipReason, startedRunIds, and blockedReasons at the top level, so background automation summaries preserve completed/skipped cause, which run started, and why candidates were blocked without parsing nested summaries.',
      'The scheduled/event Agent sweep smoke now exercises the built main SchedulerService sweep path without provider calls or Docker, proving checked=2 duplicate candidates, started=1, blocked=1 by in-sweep run-limit counting, skipReason=none, startedRunIds evidence, blockedReasons evidence, triggerRunEvidence=passed, runLimitEvidence=passed, runtimeStartRequirements=passed, targetTaskId timeline evidence, timelineEvidence=recorded, runStatusEvidence=recorded, workspace=unchanged, provider=not-called, and docker=not-started.',
    ],
    gaps: [
