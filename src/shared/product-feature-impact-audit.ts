...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Provider-native agent session gates now require both provider-native payload provider identity and normalized plan provider identity to match the selected runtime provider, plus nonempty providerCallIds identity evidence, before normalized provider tool calls can execute, so a provider payload, normalized plan, or hand-shaped proposal without provider tool-call identity evidence cannot cross the provider-native session boundary.',
      'CapabilityRegistry now also projects provider-native session readiness from static selected-runtime evidence, exposing providerNativeSessionReady=no, providerNativeSessionRequirements=2/5, providerNativeSessionMissingRequirements=provider_payload_identity,normalized_plan_identity,provider_call_ids, providerNativeFlag=enabled, providerNativeSelectedProvider=openai, providerNativePayloadProvider=missing, providerNativePlanProvider=missing, and providerNativeProviderCallIdCount=0 in the read-only Agent API provider tool readiness smoke, plus providerNativeReadySessionReady=yes and providerNativeReadySessionRequirements=5/5 when payload provider identity, normalized plan provider identity, and providerCallIds all match the selected runtime provider from structured non-executing evidence, so enabling provider-native tool calls does not imply a live provider-native session without payload and providerCallIds evidence while the ready shape remains testable without provider calls.',
