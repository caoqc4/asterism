...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'SchedulerService.proposeSchedulerDecision now also requires a concrete title, rationale, at least one nonblank option, and a nonblank proposed outcome that matches one of the options before recording panel.scheduler_decision_proposed, so scheduler/background Decision proposals entering Task Dynamics remain actionable proposal cards instead of narrative-only events or recommendations for unavailable actions.',
      'SchedulerService same-day scheduler Decision proposal dedupe now trusts only target-scoped proposal history whose timeline event taskId and payload targetTaskId both match the candidate task, so stale title-only or cross-task proposal records cannot suppress a fresh evidence-backed proposal.',
