...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Provider-native agent session gates now require both provider-native payload provider identity and normalized plan provider identity to match the selected runtime provider, plus nonempty providerCallIds and provider web/search call tool names from provider_payload evidence, before normalized provider web/search tool calls can execute, so a provider payload, normalized plan, runtime projection, or hand-shaped proposal without provider-owned web/search tool-call identity evidence cannot cross the provider-native session boundary.',
      'CapabilityRegistry now also projects provider-native session readiness from static selected-runtime evidence, exposing providerNativeSessionReady=no, providerNativeSessionRequirements=2/6, providerNativeSessionMissingRequirements=provider_payload_identity,normalized_plan_identity,provider_call_ids,provider_web_search_calls, providerNativeFlag=enabled, providerNativeSelectedProvider=openai, providerNativePayloadProvider=missing, providerNativePlanProvider=missing, providerNativeProviderCallIdCount=0, and providerNativeProviderWebSearchCallCount=0 in the read-only Agent API provider tool readiness smoke, plus providerNativeReadySessionReady=yes and providerNativeReadySessionRequirements=6/6 only when payload provider identity, normalized plan provider identity, providerCallSource=provider_payload, providerCallIds, and provider web/search call tool names all match the selected runtime provider from structured non-executing evidence, so enabling provider-native tool calls does not imply a live provider-native web/search session without payload-owned providerCallIds and web/search tool-name evidence while the ready shape remains testable without provider calls.',
