...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Subtask create-many apply plans now enrich decomposition timeline runtimeContract evidence with the apply-plan evidenceRunId and parentTaskId, and evaluateAgentApiDecompositionPromotionReadinessFromEvidence keeps selected_runtime_contract missing when those timeline runtime identity fields are absent or stitched from another run/task.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Runtime patch promotion routing now requires the approved promotion Decision artifact to match the reviewed patch artifact before promotion_decision can be ready, so a Decision for a different artifact cannot appear as a satisfied promotion Decision while other routing gates block.',
