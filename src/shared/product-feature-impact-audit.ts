...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  {
    ...
    evidence: [
      ...
      'The Agent API promotion readiness smoke now runs the shared deferred execution_run invocation, evaluateAgentApiExecutionPromotionReadiness, and evaluateAgentApiExecutionPromotionReadinessFromEvidence paths as a read-only build-gated harness, proving deferred=0/11 requirements and 0/9 gates, partial=5/11 requirements and 3/9 gates, service-evidence=3/11 requirements and 3/9 gates with providerConfigured=ready, configuredProvider=openai, providerStartupProbe=not_called, selectedRuntimeRunEvidenceChain=missing, providerPreflightRunEvidenceChain=missing, and providerPreflightTaskEvidenceChain=ready evidence until persisted same-run Run evidence exists, and synthetic-ready=11/11 requirements and 9/9 gates without provider calls or workspace writes.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Scheduled/event Agent duplicate task-source candidates now skip duplicate runtime starts in the same sweep and route a deduplicated duplicate-candidate review policy through SchedulerService.proposeSchedulerDecision with target-task identity, Standing Approval authorization, duplicateCandidateTaskIds, and duplicateCandidateDecisionProposals summary evidence.',
