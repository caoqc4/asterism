...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Provider-native agent session gates now require both provider-native payload provider identity and normalized plan provider identity to match the selected runtime provider before normalized provider tool calls can execute, so a provider payload or normalized plan from a different configured runtime cannot cross the provider-native session boundary.',
