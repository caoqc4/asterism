...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'SchedulerService.triggerScheduledEventAgentRun now returns terminalRunEvidenceStatus for the single-run operator action, so Task Dynamics can distinguish started-but-pending terminal Run evidence from completed/failed terminal evidence.',
      'SchedulerService.triggerScheduledEventAgentRun now records panel.scheduled_event_agent_triggered timeline evidence after a run starts, preserving run id, run status/outputSource/failureReason, terminalRunEvidenceStatus, target task id, Standing Approval policy id, run-limit state, runtime-start satisfied/missing requirements, schedulerTriggerServiceConnected, runtimeStartAllowed, and required trigger evidence in Task Dynamics.',
      ...
      'Scheduled/event Agent sweep results now expose skipReason, startedRunIds, blockedReasons, runtimeStartMissingRequirements, terminalRunEvidenceMissingRunIds, and triggerRunEvidenceRequired at the top level, so background automation summaries preserve completed/skipped cause, which run started, why candidates were blocked, which runtime-start requirements remain missing, which started runs still lack terminal Run evidence, and which trigger evidence is required without parsing nested summaries.',
      'The scheduled/event Agent sweep smoke now exercises the built main SchedulerService sweep path without provider calls or Docker, proving checked=2 duplicate candidates, started=1, blocked=1 by in-sweep run-limit counting, skipReason=none, startedRunIds evidence, blockedReasons evidence, runtimeStartMissingRequirements evidence, terminalRunEvidenceMissingRunIds evidence, triggerRunEvidenceRequired evidence, triggerRunEvidence=passed, runLimitEvidence=passed, runtimeStartRequirements=passed, targetTaskId timeline evidence, timelineEvidence=recorded, runStatusEvidence=recorded, workspace=unchanged, provider=not-called, and docker=not-started.',
    ],
    gaps: [
