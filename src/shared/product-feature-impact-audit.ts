...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    gaps: [
      'Future background scheduler decisions now have a SchedulerService proposal producer and Task Dynamics approval queue path, but concrete scheduler policies still need to decide when to call proposeSchedulerDecision from real run/sweep evidence instead of relying on ad hoc callers.',
    ],
    nextActions: [
      'Route concrete scheduled/event review policies through SchedulerService.proposeSchedulerDecision with run/sweep evidence, then keep durable Decision creation behind the Task Dynamics TaskplaneWritebackApprovalItem confirmation and main-side writeback dispatch boundary.',
    ],
  },
  {
