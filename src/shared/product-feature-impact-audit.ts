...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  {
    ...
    evidence: [
      ...
      'evaluateAgentApiExecutionPromotionReadinessFromEvidence now derives Agent API execution promotion readiness from structured service evidence for selected-runtime contract, target-task identity, provider-visible preflight, context manifest, context readiness step, task-memory guidance, Run Goal Contract, Write Intent extraction, reviewed-patch apply boundary, post-step verification, Run evidence persistence, and runtime gates, and now appends targetTask, runId, contextStep, contextManifest, taskMemoryGuidance, taskMemoryGuidanceCount, runGoalConditions, writeIntentActions, reviewedPatchApplyBoundary, postStepVerifier, terminalEvidence, runtimeMode, and invocationLayer identity chips so future promotion no longer depends on hand-filled requirement arrays or opaque counts.',
      ...
      'Retained API Runtime / Agent API-like RunService runs now persist an Agent API execution promotion readiness Run step from real service evidence before provider-visible execution, recording selected-runtime contract, target-task identity, provider-visible preflight, context manifest, context readiness, simplicity_check, runtime_action, pre-step, and subtask-start gates while keeping missing Write Intent extraction, reviewed-patch apply boundary, post-step verification, and terminal Run evidence explicit.',
      ...
      'Post-run Agent API execution promotion readiness now reads same-run sandbox patch promotion records through SandboxPatchPromotionRepository.listForRun and satisfies the reviewed-patch apply boundary only when real non-blocked promotion evidence exists.',
      'Post-run Agent API execution promotion readiness now has regression coverage proving parsed TASKPLANE_WRITE_INTENTS artifact.propose output plus same-run patch promotion evidence satisfies Write Intent extraction and reviewed-patch apply requirements without hand-filled readiness.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'evaluateAgentApiDecompositionPromotionReadinessFromEvidence now derives Agent API decomposition promotion readiness from structured service evidence for selected-runtime contract, parent-task identity, reversible proposal card, subtask.create_many apply plan, agent_api_decomposition source, operator confirmation boundary, and draft-only timeline evidence, keeps parent-task identity missing when the service parentTaskId and apply-plan parentTaskId do not match, and keeps the reversible proposal card missing when its parentTaskId does not match the same parent-task evidence chain.',
      'Agent API decomposition promotion readiness now returns satisfied and missing requirement lists plus promotionReady, requirements=x/7, promotionRequirements=x/7, missingRequirements=..., promotionMissingRequirements=..., proposalId, proposalParentTask, proposalTaskEvidenceChain, parentTask, applyPlanParentTask, parentTaskEvidenceChain, subtaskCount, evidenceRunId, confirmationBoundary, draftOnlyBeforeConfirmation, runtimeMode, and invocationLayer summary evidence, matching the execution promotion readiness style without opening the deferred path.',
      ...
      'Agent API project decomposition drafts now return promotionReadiness with promotionReady, requirement counts, missing requirements, selected runtime contract, proposal-card, and agent_api_decomposition apply-plan evidence before the operator confirms child creation.',
      'Right-panel AI decomposition draft readiness now projects promotionReadiness identity chips for proposalId, proposalParentTask, proposalTaskEvidenceChain, parentTask, applyPlanParentTask, parentTaskEvidenceChain, subtaskCount, evidenceRunId, confirmationBoundary, draftOnlyBeforeConfirmation, runtimeMode, and invocationLayer, so the operator can inspect the reversible proposal boundary and parent-task evidence chain before creating child tasks.',
      'The Agent API decomposition promotion readiness smoke now runs evaluateAgentApiDecompositionPromotionReadiness and evaluateAgentApiDecompositionPromotionReadinessFromEvidence as a read-only build-gated harness, proving blocked=0/7 requirements, partial=6/7 requirements with agent_api_decomposition_source missing, service-evidence=6/7 requirements with proposalId, proposalParentTask, proposalTaskEvidenceChain, parentTask, applyPlanParentTask, parentTaskEvidenceChain, subtaskCount, evidenceRunId, confirmationBoundary, draftOnlyBeforeConfirmation, runtimeMode, and invocationLayer identity evidence plus agent_api_decomposition_source missing, and synthetic-ready=7/7 requirements without provider calls, subtask creation, or workspace writes.',
  ...
  },
  {
    ...
    gaps: [
      'Future background scheduler decisions now have a SchedulerService proposal producer, Task Dynamics approval queue path, concrete failed-run recovery policy, deduplicated daily run-limit review policy, and deduplicated sweep-failure trigger review policy; broader scheduler review policies still need to decide when other run/sweep evidence deserves a Decision proposal.',
    ],
    nextActions: [
  ...
  },
  {
    ...
    evidence: [
      ...
      'evaluateRuntimePatchPromotionRoutingReadinessFromEvidence now derives runtime patch promotion routing readiness from structured service evidence for selected-runtime contract, target-task identity, patch artifact, promotion Decision, promotion preflight, explicit operator apply, same-run evidence chain, and post-apply Run evidence.',
      'Runtime patch promotion routing now treats target-task identity as an evidence chain: targetTaskId, patchArtifact.taskId, promotionDecision.taskId, promotionPreflight.taskId, and postApplyRunEvidence.taskId must all match before future runtime patch promotion can be ready.',
      'Runtime patch promotion readiness now returns satisfied and missing requirement lists plus promotionReady, requirements=x/8, promotionRequirements=x/8, promotionSatisfiedRequirements=..., missingRequirements=..., promotionMissingRequirements=..., targetTaskEvidenceChain, operatorId, patchRunId, decisionRunId, preflightRunId, postApplyRunId, sameRunId, touchedFileCount, and touchedFiles summary evidence, matching the Agent API promotion readiness style without opening direct workspace writes.',
      ...
      'Settings configuration safety rows now render sandbox.patch_promotion evidence chips for promotionReady, promotionRequirements, promotionSatisfiedRequirements, promotionMissingRequirements, missingRequirements, selectedRuntimeContract, targetTaskIdentity, targetTaskEvidenceChain, sameRunEvidenceChain, explicitOperatorApply, postApplyRunEvidence, operatorId, patchArtifactId, promotionDecisionId, patchArtifactTask, promotionDecisionTask, promotionPreflightTask, postApplyTask, patchRunId, decisionRunId, preflightRunId, postApplyRunId, sameRunId, touchedFileCount, and touchedFiles so operators can review patch promotion routing gaps and identity without parsing the full diagnostic summary.',
      'The runtime patch promotion routing readiness smoke now runs evaluateRuntimePatchPromotionRoutingReadiness and evaluateRuntimePatchPromotionRoutingReadinessFromEvidence as a read-only build-gated harness, proving blocked=2/8 requirements, same-run-blocked=7/8 requirements with same_run_evidence_chain missing, service-evidence=4/8 requirements with patchArtifactId, promotionDecisionId, patchArtifactTask, promotionDecisionTask, promotionPreflightTask, targetTaskEvidenceChain, patchRunId, decisionRunId, preflightRunId, sameRunId, and touchedFileCount identity evidence plus target_task_identity, explicit_operator_apply, same_run_evidence_chain, and post_apply_run_evidence missing, and synthetic-ready=8/8 requirements without provider calls, workspace apply, or workspace writes.',
      'SandboxPatchPromotionApplyService now appends evaluateRuntimePatchPromotionRoutingReadinessFromEvidence output to applied and already-applied audit summaries after explicit operator apply, so real workspace apply evidence records target-task identity across patch artifact, promotion Decision, preflight, and post-apply evidence; same-run patch artifact; approved promotion Decision; ready preflight; explicit operator apply; same-run evidence chain; post-apply Run evidence; and the remaining selected-runtime-contract gap for future API/runtime routing.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'AI Runtime settings now render Agent API provider tool readiness, providerToolStatus, providerToolRequirements, providerToolMissingRequirements, selectedApiRuntime, providerConfigured, startupProbe, providerOwnedMetadata, providerMetadataOwner, providerMetadataPackage, explicitToolDeclaration, explicitToolDeclarationSource, declaredToolCount, declaredWebSearchToolCount, and declaredWebSearchTools chips from the shared capability summary, so providerToolReadiness=not_declared and providerToolStatus=blocked|not_declared are visible without implying provider-native web/search/tool support.',
      'evaluateAgentApiProviderToolReadinessFromEvidence now derives Agent API provider tool readiness from structured service evidence for selected API Runtime, provider configuration, no-startup-probe policy, provider-owned metadata, and explicit provider-owned web/search/browse tool declarations, so provider configuration alone and unrelated provider-owned function tools stay providerToolReadiness=not_declared.',
      'CapabilityRegistry now derives Agent API Runtime providerToolReadiness, providerToolStatus, providerToolRequirements, and providerToolMissingRequirements through evaluateAgentApiProviderToolReadinessFromEvidence plus no-start local provider package metadata for @ai-sdk/openai and @ai-sdk/anthropic instead of hard-coded provider tool/search readiness strings.',
      'Agent API provider tool readiness summaries now expose selectedApiRuntime, providerConfigured, providerOwnedMetadata, providerMetadataOwner, providerMetadataPackage, explicitToolDeclaration, explicitToolDeclarationSource, declaredToolCount, declaredWebSearchToolCount, and declaredWebSearchTools, so settings and smokes can distinguish runtime selection, provider configuration, provider-owned package declarations, and web/search-specific declarations from runtime-probe-only tool discovery.',
      'The Agent API provider tool readiness smoke now reads the shared Agent API Runtime capability row and evaluateAgentApiProviderToolReadinessFromEvidence as a read-only build-gated harness, proving providerToolReadiness=not_declared, providerToolStatus=not_declared, providerToolRequirements=4/5, providerToolMissingRequirements=explicit_tool_declaration, providerMetadataPackage=@ai-sdk/openai, declaredWebSearchToolCount=0, declaredWebSearchTools=none, service-evidence=4/5 requirements with explicit_tool_declaration missing, startupProbe=never, executionRun=deferred, runtimeExecutable=no, provider=not-called, network=not-called, and workspace=unchanged even when API Runtime is selected and provider configuration is present.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Scheduled/event trigger plans now expose runtimeStartSatisfiedRequirements=... and runtimeStartMissingRequirements=... plus runtimeStartReady and runtimeStartRequirements=x/3 summary evidence for trigger_plan_ready, scheduler_trigger_service, and run_limit_count.',
      ...
      'The scheduled/event trigger readiness smoke now runs planScheduledEventAgentTrigger and planScheduledEventAgentTriggerFromEvidence as a read-only build-gated harness, proving no-service plans stay runtimeStartAllowed=false with runtimeStartRequirements=1/3, connected trigger service without run-limit counting stays blocked, service-evidence=1/3 runtime-start requirements with run_limit_count missing, daily-cap-reached plans stay blocked by run-limit evidence, and only trigger-plan-ready + scheduler-trigger-service + run-limit-count evidence reaches runtimeStartRequirements=3/3 without provider calls, Docker, or workspace writes.',
      ...
      'scheduler:triggerScheduledEventAgentRun now emits run.changed, task.changed, and brief.changed after a started scheduled/event Agent run, so the explicit operator action refreshes the same product surfaces as other Run start paths.',
      ...
      'The scheduled/event Agent sweep smoke now proves sweepListenerEvidence=passed, disconnectedSweepListenerEvidence=passed, inFlightSweepListenerEvidence=passed, and failedSweepListenerEvidence=passed, so the Brief refresh listener path has acceptance coverage for completed, ports_not_connected, in_flight, and sweep_failed outcomes.',
      ...
      'The scheduled/event Agent sweep smoke now exercises the built main SchedulerService sweep path without provider calls or Docker, proving checked=2 duplicate candidates, checkedTaskIdsEvidence=passed, started=1, blocked=1 by in-sweep run-limit counting, skipReason=none, startedRunIds evidence, blockedReasons evidence, blockedTaskSummaryEvidence=passed, runFailureReasons evidence, automationMissingRequirements evidence, automationSatisfiedRequirements evidence, runtimeStartMissingRequirements evidence, terminalRunEvidenceMissingRunIds evidence, triggerRunEvidenceRequired evidence, triggerRunEvidenceStatus=pending_terminal_run_evidence, manualSweepSummary evidence, terminalSweepSummary evidence, cronSweepSummary evidence, disconnectedSkipReason=ports_not_connected, disconnectedTriggerRunEvidenceStatus=not_started, inFlightSkipReason=in_flight, inFlightTriggerRunEvidenceStatus=not_started, failedSkipReason=sweep_failed, failedTriggerRunEvidenceStatus=not_started, failedSweepSummaryEvidence=recorded, failedSweepRecoveryEvidence=passed, timelineFailedStartedRunEvidence=recorded, timelineFailedNotBlockedEvidence=passed, timelineFailedTriggerRunEvidence=recorded, timelineFailedSweepSummaryEvidence=recorded, sourceFailedSkipReason=sweep_failed, sourceFailedTriggerRunEvidenceStatus=not_started, sourceFailedSweepSummaryEvidence=recorded, sourceFailedSweepRecoveryEvidence=passed, cronSoakRunLimitEvidence=passed, cronSoakAutomationReadinessEvidence=passed, cronSoakNoSecondTriggerEvidence=passed, completedSweepTimeEvidence=recorded, skippedSweepTimeEvidence=recorded, boundedRunTargetTask=passed, boundedRunTaskMemoryGuidance=passed, boundedRunAutomationReadiness=passed, boundedRunFirstCriterion=passed, boundedRunFirstSource=passed, boundedRunPostStepGuidance=passed, boundedRunWorkspaceWriteBoundary=passed, boundedRunStandingApprovalScope=passed, terminalTriggerRunEvidenceStatus=ready_for_terminal_review, cronTriggerRunEvidenceStatus=ready_for_terminal_review, manualTriggerKind=manual, terminalTriggerKind=manual, cronTriggerKind=cron, startupSweepJobConnected=yes, triggerRunEvidence=passed, sweepAutomationReadinessEvidence=passed, terminalTriggerRunEvidence=passed, cronTriggerRunEvidence=passed, cronRunFailureReasonEvidence=passed, triggerKindEvidence=passed, boundedRunTargetTaskEvidence=passed, boundedRunTaskMemoryEvidence=passed, boundedRunAutomationReadinessEvidence=passed, boundedRunFirstCriterionEvidence=passed, boundedRunFirstSourceEvidence=passed, boundedRunPostStepEvidence=passed, boundedRunWorkspaceBoundaryEvidence=passed, boundedRunStandingApprovalScopeEvidence=passed, runLimitEvidence=passed, runtimeStartRequirements=passed, targetTaskId timeline evidence, timelineEvidence=recorded, terminalTimelineEvidence=recorded, cronTimelineEvidence=recorded, timelineWorkspaceBoundary=recorded, terminalTimelineWorkspaceBoundary=recorded, cronTimelineWorkspaceBoundary=recorded, startupSweepJobEvidence=recorded, sweepSummaryEvidence=recorded, disconnectedSweepSummaryEvidence=recorded, inFlightSweepSummaryEvidence=recorded, runStatusEvidence=recorded, terminalRunStatusEvidence=recorded, cronRunStatusEvidence=recorded, workspace=unchanged, provider=not-called, and docker=not-started.',
      'Local scheduled/event Agent sweep acceptance on 2026-05-27 passed through npm run accept:scheduled-event-agent-sweep-smoke with status=completed, checked=2, started=1, blocked=1, triggerRunEvidenceStatus=pending_terminal_run_evidence, terminalTriggerRunEvidenceStatus=ready_for_terminal_review, cronTriggerRunEvidenceStatus=ready_for_terminal_review, cronRunFailureReasonEvidence=passed, durableRunLimitCountEvidence=passed, cronSoakRunLimitEvidence=passed, cronSoakNoSecondTriggerEvidence=passed, startupSweepJobConnected=yes, sweepSummaryEvidence=recorded, sweepListenerEvidence=passed, disconnectedSweepSummaryEvidence=recorded, disconnectedSweepListenerEvidence=passed, inFlightSweepSummaryEvidence=recorded, inFlightSweepListenerEvidence=passed, failedSweepSummaryEvidence=recorded, failedSweepListenerEvidence=passed, failedSweepRecoveryEvidence=passed, timelineFailedStartedRunEvidence=recorded, timelineFailedNotBlockedEvidence=passed, timelineFailedTriggerRunEvidence=recorded, timelineFailedSweepSummaryEvidence=recorded, sourceFailedSweepSummaryEvidence=recorded, sourceFailedSweepRecoveryEvidence=passed, skippedSweepTimeEvidence=recorded, triggerKindEvidence=passed, boundedRunTargetTaskEvidence=passed, boundedRunTaskMemoryEvidence=passed, boundedRunAutomationReadinessEvidence=passed, boundedRunFirstCriterionEvidence=passed, boundedRunFirstSourceEvidence=passed, boundedRunPostStepEvidence=passed, boundedRunWorkspaceBoundaryEvidence=passed, boundedRunStandingApprovalScopeEvidence=passed, timelineWorkspaceBoundary=recorded, terminalTimelineWorkspaceBoundary=recorded, cronTimelineWorkspaceBoundary=recorded, startupSweepJobEvidence=recorded, terminalRunEvidenceMissingRunIds=none, workspace=unchanged, provider=not-called, and docker=not-started.',
    ],
    gaps: [
