...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'AI Runtime settings now render Agent API provider tool readiness, providerToolStatus, providerToolRequirements, providerToolMissingRequirements, selectedApiRuntime, providerConfigured, startupProbe, providerOwnedMetadata, providerMetadataOwner, explicitToolDeclaration, explicitToolDeclarationSource, and declaredToolCount chips from the shared capability summary, so providerToolReadiness=not_declared and providerToolStatus=blocked|not_declared are visible without implying provider-native web/search/tool support.',
      ...
      'Agent API provider tool readiness summaries now expose providerMetadataOwner, explicitToolDeclarationSource, and declaredToolCount, so settings and smokes can distinguish provider-owned declarations from runtime-probe-only tool discovery.',
      'The Agent API provider tool readiness smoke now reads the shared Agent API Runtime capability row and evaluateAgentApiProviderToolReadinessFromEvidence as a read-only build-gated harness, proving providerToolReadiness=not_declared, providerToolStatus=not_declared, providerToolRequirements=3/5, providerToolMissingRequirements=provider_owned_metadata,explicit_tool_declaration, service-evidence=3/5 requirements with provider_owned_metadata and explicit_tool_declaration missing, startupProbe=never, executionRun=deferred, runtimeExecutable=no, provider=not-called, network=not-called, and workspace=unchanged even when API Runtime is selected and provider configuration is present.',
