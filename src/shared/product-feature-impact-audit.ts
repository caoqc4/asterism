...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Agent API Runtime capability summaries now expose executionRunPromotionRequirements=0/11, executionRunMissingRequirements=..., executionRunMissingGates=..., decompositionPromotionRequirements=0/7, decompositionMissingRequirements=..., decompositionPromotionMissingRequirements=..., providerToolReadiness=not_declared, providerToolStatus=blocked|not_declared|declared, and startupProbe=never, so provider tool/search readiness is not implied by provider configuration or checked through startup calls.',
      'AI Runtime settings now render Agent API execution_run readiness chips from the shared capability summary, making promotion count, missing requirement count, missing requirement list, key gate count, key gate list, missing gate count, and missing gate list visible without parsing the diagnostic text.',
