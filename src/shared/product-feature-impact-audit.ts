...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'CapabilityRegistry now derives Agent API Runtime providerToolReadiness, providerToolRequirements, and providerToolMissingRequirements through evaluateAgentApiProviderToolReadinessFromEvidence instead of hard-coded provider tool/search readiness strings.',
      'The Agent API provider tool readiness smoke now reads the shared Agent API Runtime capability row and evaluateAgentApiProviderToolReadinessFromEvidence as a read-only build-gated harness, proving providerToolReadiness=not_declared, providerToolRequirements=3/5, providerToolMissingRequirements=provider_owned_metadata,explicit_tool_declaration, service-evidence=3/5 requirements with provider_owned_metadata and explicit_tool_declaration missing, startupProbe=never, executionRun=deferred, runtimeExecutable=no, provider=not-called, network=not-called, and workspace=unchanged even when API Runtime is selected and provider configuration is present.',
