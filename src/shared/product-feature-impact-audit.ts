...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  {
    ...
    evidence: [
      ...
      'The opt-in Agent API execution preflight smoke verifies provider-visible text-call readiness through the shared provider mapping while defaulting to provider=not-called, executionRun=deferred, promotionRequirements=0/11, requiredGates=0/9, and workspace=unchanged.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Agent API decomposition promotion readiness now returns satisfied and missing requirement lists plus requirements=x/7 summary evidence, matching the execution promotion readiness style without opening the deferred path.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'SchedulerService.triggerScheduledEventAgentRun now provides a narrow main-side trigger-service connection: it requires an injected Code Agent trigger port, reuses Standing Approval and persisted same-day run-limit checks, starts only ready plans with schedulerTriggerServiceConnected=true, and emits a bounded model-producer Code Agent run request with operatorConfirmed=true plus run-limit evidence.',
      ...
      'The scheduled/event Agent sweep smoke now exercises the built main SchedulerService sweep path without provider calls or Docker, proving checked=2 duplicate candidates, started=1, blocked=1 by in-sweep run-limit counting, triggerRunEvidence=passed, runLimitEvidence=passed, targetTaskId timeline evidence, timelineEvidence=recorded, runStatusEvidence=recorded, workspace=unchanged, provider=not-called, and docker=not-started.',
    ],
    gaps: [
