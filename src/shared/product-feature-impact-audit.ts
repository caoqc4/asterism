...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    gaps: [
      'Future Agent API decomposition generation is still not the primary task-bound runtime path; if promoted, it should prove the selected-runtime contract and surface the same reversible proposal card before confirmation.',
    ],
    nextActions: [
  ...
  },
  {
    ...
    evidence: [
      ...
      'evaluateRuntimePatchPromotionRoutingReadiness now keeps future API/runtime-generated patch promotion blocked unless the path includes a selected-runtime contract, same-run patch artifact, promotion Decision, promotion preflight, explicit operator apply, and post-apply Run evidence.',
    ],
    gaps: [
      'Future API/runtime-generated patch promotion still needs to prove the selected-runtime contract and reuse the reviewed-patch apply workflow and same-run evidence chain; direct workspace-write runtime modes remain intentionally separate from the common run path.',
    ],
    nextActions: [
      'Keep explicit apply as the product-controlled mutation path and require evaluateRuntimePatchPromotionRoutingReadiness before routing future runtime writes into selected-runtime, same-run patch artifacts, promotion Decisions, promotion preflight, explicit apply, and post-apply Run evidence.',
    ],
  },
  {
  ...
  },
  {
    ...
    evidence: [
      ...
      'planScheduledEventAgentTrigger now acts as the shared scheduled/event trigger planner: it consumes confirmed Standing Approval Task Dynamics records, re-checks runtime readiness, task readiness, policy expiry/scope/risk, task automation class, and returns runtimeStartAllowed=true only when a dedicated trigger service is connected.',
      ...
      'SchedulerService.triggerScheduledEventAgentRun now records panel.scheduled_event_agent_triggered timeline evidence after a run starts, preserving run id, target task id, Standing Approval policy id, run-limit state, schedulerTriggerServiceConnected, runtimeStartAllowed, and required trigger evidence in Task Dynamics.',
