...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'RuntimeEventRecord now formats panel.scheduled_event_agent_triggered payloads into readable Task Dynamics detail with run id, target task id, trigger plan summary, run status, terminal Run evidence status, trigger evidence status, runtime-start gate status, trigger kind, Standing Approval policy id, run-limit usage, and workspace proposal-mode write boundary.',
