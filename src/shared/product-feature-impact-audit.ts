...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Provider-native agent session gates now require both provider-native payload provider identity and normalized plan provider identity to match the selected runtime provider, plus nonempty providerCallIds, provider web/search call tool names from provider_payload evidence, and matching trusted provider-owned web/search declarations before normalized provider web/search tool calls can execute, so a provider payload, normalized plan, runtime projection, or hand-shaped proposal without provider-owned web/search tool-call identity and declaration evidence cannot cross the provider-native session boundary.',
      'CapabilityRegistry now also projects provider-native session readiness from static selected-runtime evidence, exposing providerNativeSessionReady=no, providerNativeSessionRequirements=2/7, providerNativeSessionMissingRequirements=provider_payload_identity,normalized_plan_identity,provider_call_ids,provider_web_search_calls,provider_web_search_declaration, providerNativeFlag=enabled, providerNativeSelectedProvider=openai, providerNativePayloadProvider=missing, providerNativePlanProvider=missing, providerNativeProviderCallIdCount=0, providerNativeProviderWebSearchCallCount=0, and trusted/untrusted provider web-search declaration evidence in the read-only Agent API provider tool readiness smoke, plus providerNativeReadySessionReady=yes and providerNativeReadySessionRequirements=7/7 only when payload provider identity, normalized plan provider identity, providerCallSource=provider_payload, providerCallIds, provider web/search call tool names, and trusted provider-owned web/search declarations all match the selected runtime provider from structured non-executing evidence, so enabling provider-native tool calls does not imply a live provider-native web/search session without payload-owned providerCallIds and provider-owned web/search tool-name declaration evidence while the ready shape remains testable without provider calls.',
