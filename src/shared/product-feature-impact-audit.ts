...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Brief automatic-sweep chips now derive the visible label from lastScheduledEventAgentSweepSummary before the timestamp, so completed sweeps show 已运行, waiting_for_first_tick stays 已接线, ports_not_connected shows 未接线, and in_flight shows 运行中 instead of treating every timestamped skipped sweep as a completed run.',
      'Brief completed automatic-sweep chips now include checked, started, and blocked counts parsed from lastScheduledEventAgentSweepSummary, so operators can see the last sweep scope plus whether it started work or hit blocked tasks without opening the tooltip.',
