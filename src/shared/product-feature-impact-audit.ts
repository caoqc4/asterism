...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Scheduler/background Decision proposal plans now return satisfied and missing requirement lists plus proposalReady, requirements=x/3, proposalRequirements=x/3, approvalQueueSurface, missingRequirements=..., and proposalMissingRequirements=... summary evidence without opening Decision persistence, writeback dispatch, or scheduler triggers.',
      'CapabilityRegistry now includes scheduler/background Decision proposal readiness in the runtime.scheduler summary through planSchedulerDecisionProposalFromEvidence, so ConfigurationSafetyReport surfaces the approval queue, target-task, authorization, no-persistence, no-writeback, and no-trigger boundary in settings diagnostics.',
      'The scheduler Decision proposal readiness smoke now runs planSchedulerDecisionProposal and planSchedulerDecisionProposalFromEvidence as a read-only build-gated harness, proving blocked=0/3 requirements, operator-confirmed=3/3 requirements, standing-approval=3/3 requirements, and service-evidence=2/3 requirements with approvalQueueSurface=task_dynamics plus authorization missing when Standing Approval scope does not match the target task, while keeping decisionPersistenceAllowed=false, writebackDispatchAllowed=false, and schedulerTriggerAllowed=false without provider calls, Decision persistence, writeback dispatch, scheduler triggers, or workspace writes.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'AI Runtime settings now render Agent API provider tool readiness and providerToolStatus chips from the shared capability summary, so providerToolReadiness=not_declared and providerToolStatus=blocked|not_declared are visible without implying provider-native web/search/tool support.',
