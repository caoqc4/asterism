...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Task Dynamics scheduler Decision proposal consumption now reuses shared decision_payload readiness before approval queue creation, requiring explicit payload targetTaskId plus timeline event taskId to both match the current task, normalized nonblank title, rationale, duplicate-free option labels, and a proposed outcome that canonicalizes to one of the options, so malformed, cross-task, missing-target, or legacy panel.scheduler_decision_proposed events cannot bypass the SchedulerService producer validation.',
