...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  {
    ...
    evidence: [
      ...
      'evaluateAgentApiExecutionPromotionReadinessFromEvidence now derives Agent API execution promotion readiness from structured service evidence for selected-runtime contract, target-task identity, provider-visible preflight, context manifest, context readiness step, task-memory guidance, Run Goal Contract, Write Intent extraction, reviewed-patch apply boundary, post-step verification, Run evidence persistence, and runtime gates, requires post-run Run evidence task identity to match targetTaskId before target_task_identity can stay ready, requires provider_visible_preflight to carry configured provider identity instead of only providerConfigured=true, requires write_intent_extraction to include both artifact.propose and task_file.propose with same-run and target-task identity evidence before future API execution can satisfy the reviewable writeback boundary, and now appends targetTask, runEvidenceTask, targetTaskEvidenceChain, providerConfigured, configuredProvider, providerStartupProbe, runId, writeIntentRun, writeIntentRunEvidenceChain, writeIntentTask, writeIntentTaskEvidenceChain, contextStep, contextManifest, taskMemoryGuidance, taskMemoryGuidanceCount, runGoalConditions, writeIntentActions, reviewedPatchApplyBoundary, postStepVerifier, terminalEvidence, runtimeMode, and invocationLayer identity chips so future promotion no longer depends on hand-filled requirement arrays or opaque counts.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Right-panel AI decomposition draft readiness now projects promotionReadiness identity chips for proposalId, expectedProposalId, proposalIdEvidenceChain, proposalParentTask, proposalTaskEvidenceChain, proposalSubtaskCount, applyPlanSubtaskCount, proposalSubtaskEvidenceChain, proposalSubtaskTitles, applyPlanSubtaskTitles, proposalSubtaskIdentityChain, parentTask, applyPlanParentTask, parentTaskEvidenceChain, subtaskCount, evidenceRunId, confirmationBoundary, draftOnlyBeforeConfirmation, runtimeMode, and invocationLayer, so the operator can inspect the reversible proposal boundary, proposal-id chain, subtask-count chain, subtask-title identity chain, and parent-task evidence chain before creating child tasks.',
      'The Agent API decomposition promotion readiness smoke now runs evaluateAgentApiDecompositionPromotionReadiness and evaluateAgentApiDecompositionPromotionReadinessFromEvidence as a read-only build-gated harness, proving blocked=0/7 requirements, partial=6/7 requirements with agent_api_decomposition_source missing, service-evidence=6/7 requirements with proposalId, expectedProposalId, proposalIdEvidenceChain, proposalParentTask, proposalTaskEvidenceChain, proposalSubtaskCount, applyPlanSubtaskCount, proposalSubtaskEvidenceChain, proposalSubtaskTitles, applyPlanSubtaskTitles, proposalSubtaskIdentityChain, parentTask, applyPlanParentTask, parentTaskEvidenceChain, subtaskCount, evidenceRunId, confirmationBoundary, draftOnlyBeforeConfirmation, runtimeMode, and invocationLayer identity evidence plus agent_api_decomposition_source missing, and synthetic-ready=7/7 requirements without provider calls, subtask creation, or workspace writes.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Scheduled/event Agent timeline evidence write failures after a Run starts now route a deduplicated timeline-failure review policy through SchedulerService.proposeSchedulerDecision with the started Run id, target-task identity, trigger-plan evidence, Standing Approval authorization, and timelineFailureDecisionProposals summary evidence while preserving pending terminal Run evidence for recovery.',
      'Scheduler stale-run recovery now routes each recovered run through SchedulerService.proposeSchedulerDecision with local_recovery authorization when Task Dynamics timeline evidence is connected, recording staleRunRecoveryDecisionProposals summary evidence while still marking the stale run failed locally and keeping durable Decision creation behind approval.',
    ...
    gaps: [
      'Future background scheduler decisions now have a SchedulerService proposal producer, Task Dynamics approval queue path, deduplicated failed-run recovery policy, deduplicated daily run-limit review policy, deduplicated automation-readiness review policy, deduplicated sweep-failure trigger review policy, and deduplicated timeline-failure review policy; broader scheduler review policies still need to decide when other run/sweep evidence deserves a Decision proposal.',
    ],
    nextActions: [
  ...
  },
  {
    ...
    evidence: [
      ...
      'evaluateRuntimePatchPromotionRoutingReadinessFromEvidence now derives runtime patch promotion routing readiness from structured service evidence for selected-runtime contract, target-task identity, patch artifact, promotion Decision, promotion preflight, checkpoint evidence chain, explicit operator apply, same-run evidence chain, and post-apply Run evidence.',
      ...
      'Runtime patch promotion routing now requires expected and touched file evidence to be duplicate-free before touchedFileEvidenceChain can be ready, and SandboxPatchPromotionApplyService blocks duplicate patch file entries before writing workspace files.',
      'Runtime patch promotion readiness now returns satisfied and missing requirement lists plus promotionReady, requirements=x/8, promotionRequirements=x/8, promotionSatisfiedRequirements=..., missingRequirements=..., promotionMissingRequirements=..., targetTaskEvidenceChain, checkpointEvidenceChain, operatorId, patchRunId, decisionRunId, preflightRunId, postApplyRunId, sameRunId, expectedFileCount, expectedFiles, touchedFileCount, touchedFiles, and touchedFileEvidenceChain summary evidence, matching the Agent API promotion readiness style without opening direct workspace writes.',
      ...
      'Settings configuration safety rows now render sandbox.patch_promotion evidence chips for promotionReady, promotionRequirements, promotionSatisfiedRequirements, promotionMissingRequirements, missingRequirements, selectedRuntimeContract, targetTaskIdentity, targetTaskEvidenceChain, checkpointEvidenceChain, sameRunEvidenceChain, explicitOperatorApply, postApplyRunEvidence, operatorId, patchArtifactId, promotionDecisionId, promotionCheckpointId, preflightCheckpointId, patchArtifactTask, promotionDecisionTask, promotionPreflightTask, postApplyTask, patchRunId, decisionRunId, preflightRunId, postApplyRunId, sameRunId, expectedFileCount, expectedFiles, touchedFileCount, touchedFiles, and touchedFileEvidenceChain so operators can review patch promotion routing gaps and identity without parsing the full diagnostic summary.',
      'The runtime patch promotion routing readiness smoke now runs evaluateRuntimePatchPromotionRoutingReadiness and evaluateRuntimePatchPromotionRoutingReadinessFromEvidence as a read-only build-gated harness with stale-build detection, proving blocked=2/8 requirements, same-run-blocked=7/8 requirements with same_run_evidence_chain missing, service-evidence=4/8 requirements with patchArtifactId, promotionDecisionId, patchArtifactTask, promotionDecisionTask, promotionPreflightTask, targetTaskEvidenceChain, checkpointEvidenceChain, patchRunId, decisionRunId, preflightRunId, sameRunId, expectedFileCount, expectedFiles, touchedFileCount, and touchedFileEvidenceChain identity evidence plus target_task_identity, explicit_operator_apply, same_run_evidence_chain, and post_apply_run_evidence missing, and synthetic-ready=8/8 requirements without provider calls, workspace apply, or workspace writes.',
