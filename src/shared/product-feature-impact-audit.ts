...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  {
    ...
    evidence: [
      ...
      'Right-panel Agent CLI progress now distinguishes native workspace_write capability events as reviewable write candidates that require reviewed patch, task-file proposal, or promotion evidence instead of presenting them as ordinary workspace activity.',
      'Completed native run chat summaries now mention local command or workspace activity as well as web research activity, while workspace_write steps are prioritized as reviewable write candidates instead of ordinary local activity even when web activity is also present.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'AI Runtime settings surfaces those declarations as per-runtime capability chips before execution, including visible native search readiness labels, hooks, subagents, memory, compact, clear, and write boundaries.',
