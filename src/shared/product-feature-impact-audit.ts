...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    gaps: [
      'Routine/event-triggered Agent task execution now has a narrow trigger-service connection, explicit operator IPC, Task Dynamics launch action, trigger timeline evidence, background scheduler job wiring, a local sweep smoke, and one opt-in provider-backed background live smoke, but it still needs broader runtime coverage and packaged/persistent-app soak evidence before it can count as complete L2 automatic native runtime start.',
    ],
    nextActions: [
      'Broaden scheduled/event runtime coverage from the one opt-in provider-backed smoke into packaged/persistent-app soak evidence that proves context readiness, task-memory, subtask_start, durable run-limit counting, terminal Run evidence, and post-step gates across the selected native runtime path.',
    ],
  },
  {
