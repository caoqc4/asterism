...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Provider-native agent session gates now require the provider-native payload provider identity to match the selected runtime provider before normalized provider tool calls can execute, so a provider payload from a different configured runtime cannot cross the provider-native session boundary.',
