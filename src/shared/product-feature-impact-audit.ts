...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  {
    ...
    evidence: [
      ...
      'evaluateAgentApiExecutionPromotionReadinessFromEvidence now derives Agent API execution promotion readiness from structured service evidence for selected-runtime contract, target-task identity, provider-visible preflight, context manifest, context readiness step, task-memory guidance, Run Goal Contract, Write Intent extraction, reviewed-patch apply boundary, post-step verification, Run evidence persistence, and runtime gates, requires post-run Run evidence task identity to match targetTaskId before target_task_identity can stay ready, requires provider_visible_preflight to carry configured provider identity plus same-run and target-task identity evidence instead of only providerConfigured=true, requires write_intent_extraction to include both artifact.propose and task_file.propose with same-run and target-task identity evidence before future API execution can satisfy the reviewable writeback boundary, and now appends targetTask, runEvidenceTask, targetTaskEvidenceChain, providerConfigured, configuredProvider, providerStartupProbe, providerPreflightRun, providerPreflightRunEvidenceChain, providerPreflightTask, providerPreflightTaskEvidenceChain, runId, writeIntentRun, writeIntentRunEvidenceChain, writeIntentTask, writeIntentTaskEvidenceChain, contextStep, contextManifest, taskMemoryGuidance, taskMemoryGuidanceCount, runGoalConditions, writeIntentActions, reviewedPatchApplyBoundary, postStepVerifier, terminalEvidence, runtimeMode, and invocationLayer identity chips so future promotion no longer depends on hand-filled requirement arrays or opaque counts.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Runtime patch promotion routing now also treats expected patch files and post-apply touched files as one filePathSafetyChain, so future runtime patch promotion cannot satisfy patch artifact or post-apply evidence with unsafe workspace paths even when the file sets match.',
