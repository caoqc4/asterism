...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'Agent API Runtime capability summaries now expose executionRunPromotionRequirements=0/11, executionRunMissingRequirements=..., providerToolReadiness=not_declared, and startupProbe=never, so provider tool/search readiness is not implied by provider configuration or checked through startup calls.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Scheduled/event Agent sweep results now expose skipReason, startedRunIds, blockedReasons, and runtimeStartMissingRequirements at the top level, so background automation summaries preserve completed/skipped cause, which run started, why candidates were blocked, and which runtime-start requirements remain missing without parsing nested summaries.',
      'The scheduled/event Agent sweep smoke now exercises the built main SchedulerService sweep path without provider calls or Docker, proving checked=2 duplicate candidates, started=1, blocked=1 by in-sweep run-limit counting, skipReason=none, startedRunIds evidence, blockedReasons evidence, runtimeStartMissingRequirements evidence, triggerRunEvidence=passed, runLimitEvidence=passed, runtimeStartRequirements=passed, targetTaskId timeline evidence, timelineEvidence=recorded, runStatusEvidence=recorded, workspace=unchanged, provider=not-called, and docker=not-started.',
    ],
    gaps: [
