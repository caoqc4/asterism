...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  {
    ...
    evidence: [
      ...
      'Context readiness now requires credible fresh Source Context evidence for latest/current external requests, rejects stale, future-dated, or low-credibility source evidence, and still honors explicit no-research opt-outs before self_research routing.',
      ...
      'Deferred Agent API execution_run invocations now carry the future provider-visible execution required gates, including context_readiness and post_step, as structured metadata rather than text-only rationale.',
      'Agent API capability registry diagnostics now surface deferred execution_run key gates, so settings and safety reports can show context_readiness/post_step without parsing invocation text.',
