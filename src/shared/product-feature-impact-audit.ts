...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'A manual scheduled/event Agent packaged background soak preflight is now packaged behind TASKPLANE_RUN_SCHEDULED_EVENT_AGENT_PACKAGED_BACKGROUND_SOAK=true; by default it reports status=skip, packagedApp=not-launched, backgroundLiveRun=not-started, provider=not-called, docker=not-started, and workspace=unchanged, and when explicitly enabled it checks macOS, the packaged app executable, provider gates, and workspace root before launching the persistent app boundary.',
    ...
    gaps: [
      'Routine/event-triggered Agent task execution now has a narrow trigger-service connection, explicit operator IPC, Task Dynamics launch action, trigger timeline evidence, background scheduler job wiring, a local sweep smoke, one opt-in provider-backed background live smoke, and a packaged-soak preflight boundary, but it still needs a passing packaged/persistent-app soak before it can count as complete L2 automatic native runtime start.',
    ],
    nextActions: [
      'Run the packaged scheduled/event background soak after npm run dist:mac:dir to prove context readiness, task-memory, subtask_start, durable run-limit counting, terminal Run evidence, and post-step gates inside the persistent app boundary.',
    ],
  },
  {
