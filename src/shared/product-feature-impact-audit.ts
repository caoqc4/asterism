...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  {
    ...
    evidence: [
      ...
      'Retained API Runtime / Agent API-like RunService runs now persist an Agent API execution promotion readiness Run step from real service evidence before provider-visible execution, recording selected-runtime contract, target-task identity, provider-visible preflight, context manifest, context readiness, pre-step and subtask-start gates while keeping missing Write Intent extraction, reviewed-patch apply boundary, post-step verification, and terminal Run evidence explicit.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'The Agent API decomposition promotion readiness smoke now runs evaluateAgentApiDecompositionPromotionReadiness and evaluateAgentApiDecompositionPromotionReadinessFromEvidence as a read-only build-gated harness, proving blocked=0/7 requirements, partial=6/7 requirements with agent_api_decomposition_source missing, service-evidence=6/7 requirements with proposalId, parentTask, subtaskCount, evidenceRunId, confirmationBoundary, draftOnlyBeforeConfirmation, runtimeMode, and invocationLayer identity evidence plus agent_api_decomposition_source missing, and synthetic-ready=7/7 requirements without provider calls, subtask creation, or workspace writes.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Settings configuration safety rows now render sandbox.patch_promotion evidence chips for promotionReady, promotionRequirements, promotionMissingRequirements, missingRequirements, selectedRuntimeContract, targetTaskIdentity, sameRunEvidenceChain, explicitOperatorApply, postApplyRunEvidence, operatorId, patchArtifactId, promotionDecisionId, patchRunId, decisionRunId, preflightRunId, postApplyRunId, sameRunId, touchedFileCount, and touchedFiles so operators can review patch promotion routing gaps and identity without parsing the full diagnostic summary.',
      'The runtime patch promotion routing readiness smoke now runs evaluateRuntimePatchPromotionRoutingReadiness and evaluateRuntimePatchPromotionRoutingReadinessFromEvidence as a read-only build-gated harness, proving blocked=2/8 requirements, same-run-blocked=7/8 requirements with same_run_evidence_chain missing, service-evidence=5/8 requirements with patchArtifactId, promotionDecisionId, patchRunId, decisionRunId, preflightRunId, sameRunId, and touchedFileCount identity evidence plus explicit_operator_apply, same_run_evidence_chain, and post_apply_run_evidence missing, and synthetic-ready=8/8 requirements without provider calls, workspace apply, or workspace writes.',
      'SandboxPatchPromotionApplyService now appends evaluateRuntimePatchPromotionRoutingReadinessFromEvidence output to applied and already-applied audit summaries after explicit operator apply, so real workspace apply evidence records target-task identity, same-run patch artifact, approved promotion Decision, ready preflight, explicit operator apply, same-run evidence chain, post-apply Run evidence, and the remaining selected-runtime-contract gap for future API/runtime routing.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Agent API Runtime capability summaries now expose executionRunPromotionReady=no, executionRunPromotionRequirements=0/11, executionRunGateRequirements=0/9, executionRunMissingRequirements=..., executionRunPromotionMissingRequirements=..., executionRunPromotionMissingGates=..., executionRunMissingGates=..., decompositionPromotionReady=no, decompositionPromotionRequirements=0/7, decompositionMissingRequirements=..., decompositionPromotionMissingRequirements=..., providerToolReadiness=not_declared, providerToolStatus=blocked|not_declared|declared, selectedApiRuntime, providerConfigured, providerOwnedMetadata, explicitToolDeclaration, and startupProbe=never, so provider tool/search readiness is not implied by provider configuration or checked through startup calls.',
      'AI Runtime settings now render Agent API execution_run readiness chips from the shared capability summary, making promotion ready state, promotion count, gate requirement count, missing requirement count, missing requirement list, promotion missing requirement list, promotion missing gate list, key gate count, key gate list, missing gate count, and missing gate list visible without parsing the diagnostic text.',
      'AI Runtime settings now render Agent API decomposition readiness chips from the shared capability summary, making promotion ready state, promotion count, missing requirement count, missing requirement list, and promotion missing requirement list visible without parsing the diagnostic text.',
      'AI Runtime settings now render Agent API provider tool readiness, providerToolStatus, providerToolRequirements, providerToolMissingRequirements, selectedApiRuntime, providerConfigured, startupProbe, providerOwnedMetadata, providerMetadataOwner, providerMetadataPackage, explicitToolDeclaration, explicitToolDeclarationSource, and declaredToolCount chips from the shared capability summary, so providerToolReadiness=not_declared and providerToolStatus=blocked|not_declared are visible without implying provider-native web/search/tool support.',
      'evaluateAgentApiProviderToolReadinessFromEvidence now derives Agent API provider tool readiness from structured service evidence for selected API Runtime, provider configuration, no-startup-probe policy, provider-owned metadata, and explicit tool declarations, so provider configuration alone stays providerToolReadiness=not_declared.',
      ...
      'Agent API provider tool readiness summaries now expose selectedApiRuntime, providerConfigured, providerOwnedMetadata, providerMetadataOwner, providerMetadataPackage, explicitToolDeclaration, explicitToolDeclarationSource, and declaredToolCount, so settings and smokes can distinguish runtime selection, provider configuration, and provider-owned package declarations from runtime-probe-only tool discovery.',
      'The Agent API provider tool readiness smoke now reads the shared Agent API Runtime capability row and evaluateAgentApiProviderToolReadinessFromEvidence as a read-only build-gated harness, proving providerToolReadiness=not_declared, providerToolStatus=not_declared, providerToolRequirements=3/5, providerToolMissingRequirements=provider_owned_metadata,explicit_tool_declaration, providerMetadataPackage=missing, service-evidence=3/5 requirements with provider_owned_metadata and explicit_tool_declaration missing, startupProbe=never, executionRun=deferred, runtimeExecutable=no, provider=not-called, network=not-called, and workspace=unchanged even when API Runtime is selected and provider configuration is present.',
