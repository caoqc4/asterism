...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  {
    ...
    evidence: [
      ...
      'Explicit right-panel Agent API execution requests now pass the bounded Pilot decision snapshot into RunService and persist it in the Agent API execution promotion readiness step input, so run evidence can tie the operator-facing request to the product control-layer decision instead of relying only on prompt text.',
