...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      'The scheduled/event Agent sweep smoke now exercises the built main SchedulerService sweep path without provider calls or Docker, proving checked=2 duplicate candidates, started=1, blocked=1 by in-sweep run-limit counting, skipReason=none, startedRunIds evidence, blockedReasons evidence, runtimeStartMissingRequirements evidence, terminalRunEvidenceMissingRunIds evidence, triggerRunEvidenceRequired evidence, triggerRunEvidenceStatus=pending_terminal_run_evidence, triggerRunEvidence=passed, runLimitEvidence=passed, runtimeStartRequirements=passed, targetTaskId timeline evidence, timelineEvidence=recorded, runStatusEvidence=recorded, workspace=unchanged, provider=not-called, and docker=not-started.',
      'Local scheduled/event Agent sweep acceptance on 2026-05-27 passed through npm run accept:scheduled-event-agent-sweep-smoke with status=completed, checked=2, started=1, blocked=1, triggerRunEvidenceStatus=pending_terminal_run_evidence, workspace=unchanged, provider=not-called, and docker=not-started.',
    ],
    gaps: [
