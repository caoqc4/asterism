...
export const PRODUCT_FEATURE_IMPACT_AUDIT: ProductFeatureImpactAuditItem[] = [
  {
    ...
    evidence: [
      ...
      'evaluateAgentApiExecutionPromotionReadinessFromEvidence now derives Agent API execution promotion readiness from structured service evidence for selected-runtime contract, target-task identity, provider-visible preflight, context manifest, context readiness step, task-memory guidance, Run Goal Contract, Write Intent extraction, reviewed-patch apply boundary, post-step verification, Run evidence persistence, and runtime gates, requires post-run Run evidence task identity to match targetTaskId before target_task_identity can stay ready, requires provider_visible_preflight to carry configured provider identity instead of only providerConfigured=true, and now appends targetTask, runEvidenceTask, targetTaskEvidenceChain, providerConfigured, configuredProvider, providerStartupProbe, runId, contextStep, contextManifest, taskMemoryGuidance, taskMemoryGuidanceCount, runGoalConditions, writeIntentActions, reviewedPatchApplyBoundary, postStepVerifier, terminalEvidence, runtimeMode, and invocationLayer identity chips so future promotion no longer depends on hand-filled requirement arrays or opaque counts.',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Right-panel AI decomposition draft readiness now projects promotionReadiness identity chips for proposalId, proposalParentTask, proposalTaskEvidenceChain, proposalSubtaskCount, applyPlanSubtaskCount, proposalSubtaskEvidenceChain, parentTask, applyPlanParentTask, parentTaskEvidenceChain, subtaskCount, evidenceRunId, confirmationBoundary, draftOnlyBeforeConfirmation, runtimeMode, and invocationLayer, so the operator can inspect the reversible proposal boundary, subtask-count chain, and parent-task evidence chain before creating child tasks.',
      'The Agent API decomposition promotion readiness smoke now runs evaluateAgentApiDecompositionPromotionReadiness and evaluateAgentApiDecompositionPromotionReadinessFromEvidence as a read-only build-gated harness, proving blocked=0/7 requirements, partial=6/7 requirements with agent_api_decomposition_source missing, service-evidence=6/7 requirements with proposalId, proposalParentTask, proposalTaskEvidenceChain, proposalSubtaskCount, applyPlanSubtaskCount, proposalSubtaskEvidenceChain, parentTask, applyPlanParentTask, parentTaskEvidenceChain, subtaskCount, evidenceRunId, confirmationBoundary, draftOnlyBeforeConfirmation, runtimeMode, and invocationLayer identity evidence plus agent_api_decomposition_source missing, and synthetic-ready=7/7 requirements without provider calls, subtask creation, or workspace writes.',
