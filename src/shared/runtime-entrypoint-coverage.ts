...
export const RUNTIME_ENTRYPOINT_COVERAGE: RuntimeEntrypointCoverage[] = [
  ...
  },
  {
    ...
    notes: 'SchedulerService.triggerScheduledEventAgentRun now provides a narrow main-side trigger-service connection through an injected Code Agent run port after standing approval, run-limit counting, task class, scheduler_loop Agent Capability Gateway evidence, and runtime readiness pass. It is exposed through explicit operator IPC scheduler:triggerScheduledEventAgentRun and a 15-minute scheduler sweep that only loads scheduled/event/routine task candidates from a dedicated task-source port; the background sweep also requires the Task Dynamics timeline port before any automatic start can run. Readiness diagnostics do not start Agent CLI/API runtimes, trigger timeline evidence preserves targetTaskId plus the run status/outputSource/failureReason returned by the trigger port, automation readiness summary and satisfied/missing requirements, terminalRunEvidenceStatus, and triggerRunEvidenceStatus. The scheduler_loop gateway records selected scheme, execution runtime, fallback, CLI-first support, and schedulerLoopApiDeferred=true for selected Agent API paths, keeping future API scheduler execution deferred instead of silently starting the compatibility trigger port. The bounded Code Agent run request also carries automation readiness evidence including scheduledEventEntrypoint=available. The sweep persists lastScheduledEventAgentSweepSummary for completed, ports_not_connected, in_flight, and sweep_failed outcomes and exposes checkedTaskIds, startedRunIds, blockedReasons, blockedTaskSummaries, runFailureReasons, automationMissingRequirements, automationSatisfiedRequirements, terminalRunEvidenceMissingRunIds, triggerRunEvidenceRequired, triggerRunEvidenceStatus, sanitized error evidence for task-source/trigger-port failures, and completed/skipped/failed sweep time evidence. Timeline-recording failures after a run starts preserve startedRunIds, terminalRunEvidenceMissingRunIds, triggerRunEvidenceRequired, and pending_terminal_run_evidence instead of hiding the started run or counting it as blocked. Failed sweeps release the in-flight guard and keep automatic action health visible even when no run starts; broader automatic starts must keep scheduler configuration, context readiness, task-memory gates, target-task checks, trigger timeline evidence, sweep summary evidence, and terminal run evidence before widening runtime coverage.',
  },
  {
    id: 'run.trigger',
