...
export const AGENT_PRINCIPLES_COMPLIANCE: AgentPrinciplesComplianceItem[] = [
  ...
  },
  {
    ...
    priority: 'p0',
    implementedBy: [
      'RuntimeEntrypointCoverage keeps retained entrypoints explicit for provider-visible execution, provider-visible planning, provider-visible assistance, hidden local execution, task capture, task metadata updates, ordinary task transitions, running/completion transitions, completion-check records, hierarchy maintenance, project decomposition, decision draft, decision creation, decision action, agent tools, durable writes, product configuration, preference memory, method-library writes, scheduler maintenance, capability probes, context transitions, task dynamics, and capability checks.',
  ...
  },
  {
    ...
    priority: 'p0',
    implementedBy: [
      'TASKPLANE_CORE_AGENT_CONTEXT injects the always-loaded GoalPilot Business Advancement Framework into chat, decomposition, TextExecutor, and working context flows while phase-specific specs remain on-demand.',
      ...
      'RuntimeCapabilitySnapshot can be included in RuntimeContextManifest so model, workspace, flags, tool scaffold state, and capability-registry counts become explicit context assembly inputs.',
      'RuntimeCapabilitySnapshot preserves full tool scaffold family summaries and registry count summaries for capability consumers without exposing optional catalogue item names.',
  ...
  },
  {
    ...
    implementedBy: [
      ...
      'CrossTaskLearningBoundary keeps task-specific, batch-specific, and phase-specific corrections in Task Records and routes cross-task habits or reusable SOP-shaped text to confirmation-only proposals.',
  ...
  },
  {
    ...
    implementedBy: [
      ...
      'CanonicalDataDiagnostics reports missing canonical fields, stale legacy fallback fields, hierarchy backlink mismatches, orphan task references, blocker/dependency orphan links, blocker source-context orphan links, and missing task-scoped Decision bindings as read-only diagnostics before any repair flow mutates records.',
      'CanonicalDataWriteValidation checks write-boundary fields separately from full canonical records, and retained task-file, source-context, artifact, Decision, Work Habit, process-template, and process-template-binding create/update/apply paths reject unknown or read-only legacy fields before repository persistence.',
    ...
    gaps: [
      ...
      'CanonicalDataDiagnostics covers read-only mechanical checks, and CanonicalDataWriteValidation covers retained task-file, source-context, artifact, Decision, Work Habit, process-template, and process-template-binding write boundaries; future repository and service write paths should consume canonical field authority where write-time validation becomes mechanical.',
  ...
  },
  {
    ...
    implementedBy: [
      ...
      'RunService, CodeAgentRunService, OperatorStartedRunService, and AgentCliRunService run subtask_start target-readiness checks before creating task-bound work.',
  ...
  },
  {
    ...
    implementedBy: [
      ...
      'RunService, CodeAgentRunService, OperatorStartedRunService, and AgentCliRunService pass run_start through pre_step verification.',
      ...
      'TaskService task updates, blockers, completion criteria, and dependencies pass structured task-state writes through task_mutation and pre-step verification at the service boundary.',
      'TaskService source contexts and process-template bindings keep separate durable-write boundaries while still resolving the owning task before task_mutation verification.',
      ...
      'RunService, CodeAgentRunService, OperatorStartedRunService, and AgentCliRunService also pass target-task readiness through subtask_start before run creation.',
      ...
      'Tasks file content saves and project decomposition writes use renderer runtime guards before durable persistence, while completion-criteria writes are covered by the structured task-state mutation path.',
      ...
      'RuntimeEntrypointCoverage keeps retained execution, planning, assistance, resume, capability-probe, context-transition, task-capture, task-metadata, ordinary task-transition, running/completion transition, completion-check records, project-decomposition, decision-draft, decision-write, decision-action, agent-tool, product-configuration, preference-memory, method-library, scheduler-maintenance, and durable-write entrypoints explicit with required runtime gates.',
      'RuntimeEntrypointCoverage separates structured task-state resource writes from task file/source/artifact writes, preventing blocker, completion-criteria, and dependency mutations from being treated as file persistence.',
  ...
  },
  {
    ...
    implementedBy: [
      ...
      'RightPanel consumes RuntimeHandoff for protected refresh, leave-context, start-global, task-switch confirmation, and phase-closeout child handoff.',
  ...
  },
  {
    ...
    implementedBy: [
      ...
      'RightPanel distinguishes protected refresh, leave task context, and new conversation.',
      ...
      'RuntimeHandoffPreview represents refresh/archive preview as reusable runtime data instead of RightPanel-only text assembly.',
      ...
      'RightPanel task-session transitions now share one reducer for active task, pending switch, input, captured-task confirmation, phase closeout, and task-file proposal state.',
    ],
    gaps: [
