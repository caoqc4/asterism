...
export const RUNTIME_LIFECYCLE_COVERAGE: RuntimeLifecycleCoverageItem[] = [
  ...
  },
  {
    ...
    coveredBy: [
      ...
      'Gmail OAuth now has a service-layer credential path: refresh tokens are stored in keychain, access tokens are refreshed on demand, authorization URLs use PKCE/state, loopback callbacks validate state, and an authorization session composes those pieces without opening a browser or changing UI.',
    ...
    gaps: [
      ...
      'External Access now has a structured connector status service, adapter contract, opt-in local inbox adapter, opt-in Gmail adapter, and Gmail OAuth service-layer token/session plumbing; UI connect/disconnect actions, browser launch, revoke handling, and live connector smoke coverage remain future work.',
    ...
    nextImplementation: [
      ...
      'Expose Gmail OAuth connect/disconnect only from External Access after browser launch, revoke, and confirmation boundaries are wired to the existing OAuth session service.',
