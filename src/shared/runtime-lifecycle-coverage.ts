...
export const RUNTIME_LIFECYCLE_COVERAGE: RuntimeLifecycleCoverageItem[] = [
  ...
  },
  {
    ...
    coveredBy: [
      ...
      'MemorySurfaceWriteCoverage binds retained memory-write IPC channels for task files, source contexts, artifacts, Decisions, and Work Habits to the same surface coverage matrix.',
  ...
  },
  {
    ...
    coveredBy: [
      ...
      'CanonicalDataContract documents authoritative fields, write/read authority, legacy fallback conditions, and repair routes for task hierarchy, task files, sources, artifacts, Decisions, blockers, dependencies, run events, task dynamics, Work Habits, and process templates.',
    ...
    gaps: [
      ...
      'CanonicalDataContract now has read-only mechanical diagnostics and write-boundary validation for retained task-file, source-context, artifact, and Decision writes; future repository and service writes should consume it where write-time validation is useful.',
  ...
  },
  {
    ...
    coveredBy: [
      ...
      'Gmail OAuth now has a service-layer credential path: refresh tokens are stored in keychain, access tokens are refreshed on demand, authorization URLs use PKCE/state, loopback callbacks validate state, disconnect revokes and clears local credentials, and an authorization session composes those pieces without opening a browser or changing UI.',
    ...
    gaps: [
      ...
      'External Access now has a structured connector status service, adapter contract, opt-in local inbox adapter, opt-in Gmail adapter, and Gmail OAuth service-layer token/session/disconnect plumbing; UI connect/disconnect actions, browser launch, confirmation boundaries, and live connector smoke coverage remain future work.',
    ...
    nextImplementation: [
      ...
      'Expose Gmail OAuth connect/disconnect only from External Access after browser launch and confirmation boundaries are wired to the existing OAuth session and disconnect services.',
