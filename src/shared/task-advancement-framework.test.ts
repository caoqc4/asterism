...
describe('GoalPilot task advancement framework', () => {
  it('defines a product-level peer framework for task rhythm', () => {
    ...
    expect(TASKPLANE_TASK_ADVANCEMENT_FRAMEWORK).toContain('select the focus before judging advancement');
    expect(TASKPLANE_TASK_ADVANCEMENT_FRAMEWORK).toContain('do not re-rank the whole task pool');
