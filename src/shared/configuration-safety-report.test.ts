...
describe('configuration safety report', () => {
  ...
  it('keeps costly or mutating capabilities approval-bound', () => {
    ...
    expect(report.surfaces.find((surface) => surface.id === 'runtime.scheduler')?.diagnosticSummary)
      .toContain('runtimeStartMissingRequirements=trigger_plan_ready,selected_runtime_identity,run_limit_count');
    ...
    expect(report.surfaces.find((surface) => surface.id === 'sandbox.patch_promotion')?.diagnosticSummary)
      .toContain('operatorId=missing / operatorApplySurface=missing / operatorApplySurfaceEvidenceChain=missing / operatorApplyTask=missing / operatorApplyRun=missing / operatorApplyCheckpoint=missing / operatorApplyEvidenceChain=missing / patchRunId=missing / decisionRunId=missing / preflightRunId=missing / postApplyRunId=missing / sameRunId=missing / expectedFileCount=0 / expectedFiles=none / expectedFileEvidenceChain=missing / touchedFileCount=0 / touchedFiles=none / postApplyFilesMatched=no / filePathSafetyChain=missing / touchedFileEvidenceChain=missing');
  });
