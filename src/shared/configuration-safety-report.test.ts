...
describe('configuration safety report', () => {
  ...
  it('keeps costly or mutating capabilities approval-bound', () => {
    ...
    expect(report.surfaces.find((surface) => surface.id === 'sandbox.patch_promotion')?.diagnosticSummary)
      .toContain('operatorId=missing / patchRunId=missing / decisionRunId=missing / preflightRunId=missing / postApplyRunId=missing / sameRunId=missing / expectedFileCount=0 / expectedFiles=none / touchedFileCount=0 / touchedFiles=none / touchedFileEvidenceChain=missing');
  });
