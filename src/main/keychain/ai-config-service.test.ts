...
describe('AiConfigService', () => {
  ...
  it('returns the injected config path in status responses', async () => {
    ...
    expect(status.capabilityRegistry?.find((entry) => entry.id === 'agent_api.runtime')).toMatchObject({
      status: 'disabled',
      visibility: 'hidden',
      summary: 'executionKind=api / status=development / supportedPhases=chat,decomposition,decision,scheduled_brief / executionRun=deferred / executionRunPromotionReady=no / executionRunPromotionRequirements=0/11 / executionRunMissingRequirements=selected_runtime_contract,target_task_identity,provider_visible_preflight,runtime_context_manifest,context_readiness_step,task_memory_guidance,run_goal_contract,write_intent_extraction,reviewed_patch_apply_boundary,post_step_verification,run_evidence_persistence / executionRunPromotionMissingRequirements=selected_runtime_contract,target_task_identity,provider_visible_preflight,runtime_context_manifest,context_readiness_step,task_memory_guidance,run_goal_contract,write_intent_extraction,reviewed_patch_apply_boundary,post_step_verification,run_evidence_persistence / executionRunKeyGates=runtime_context_assembly,context_readiness,task_memory_coverage,task_memory_guidance,pre_step,subtask_start,post_step / executionRunMissingGates=runtime_context_assembly,context_readiness,task_memory_coverage,task_memory_guidance,pre_step,subtask_start,post_step / decompositionPromotionRequirements=0/7 / decompositionMissingRequirements=selected_runtime_contract,parent_task_identity,reversible_proposal_card,subtask_create_many_apply_plan,agent_api_decomposition_source,operator_confirmation_boundary,draft_only_timeline_evidence / decompositionPromotionMissingRequirements=selected_runtime_contract,parent_task_identity,reversible_proposal_card,subtask_create_many_apply_plan,agent_api_decomposition_source,operator_confirmation_boundary,draft_only_timeline_evidence / providerToolReadiness=not_declared / providerToolStatus=blocked / providerToolRequirements=1/5 / providerToolMissingRequirements=selected_api_runtime,provider_configured,provider_owned_metadata,explicit_tool_declaration / providerMetadataOwner=missing / providerMetadataPackage=missing / explicitToolDeclarationSource=missing / declaredToolCount=0 / startupProbe=never / provider=missing',
