...
describe('AiConfigService', () => {
  ...
  it('returns the injected config path in status responses', async () => {
    ...
    expect(status.capabilityRegistry?.find((entry) => entry.id === 'agent_api.runtime')).toMatchObject({
      status: 'disabled',
      visibility: 'hidden',
      summary: expect.stringContaining('providerToolReadiness=not_declared / providerToolStatus=blocked / providerToolRequirements=1/5 / providerToolMissingRequirements=selected_api_runtime,provider_configured,provider_owned_metadata,explicit_tool_declaration / selectedApiRuntime=missing / providerConfigured=missing / providerOwnedMetadata=missing / providerMetadataOwner=missing / providerMetadataPackage=missing / explicitToolDeclaration=missing / explicitToolDeclarationSource=missing / declaredToolCount=0 / declaredWebSearchToolCount=0 / declaredWebSearchTools=none / startupProbe=never / provider=missing'),
