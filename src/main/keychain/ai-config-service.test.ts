...
describe('AiConfigService', () => {
  ...
  it('returns the injected config path in status responses', async () => {
    ...
    expect(status.capabilityRegistry?.find((entry) => entry.id === 'agent_api.runtime')).toMatchObject({
      status: 'disabled',
      visibility: 'hidden',
      summary: 'executionKind=api / status=development / supportedPhases=chat,decomposition,decision,scheduled_brief / executionRun=deferred / executionRunKeyGates=context_readiness,post_step / provider=missing',
