...
describe('RunOrchestrator', () => {
  ...
  it('settles local-note sessions from terminal runtime events when present', async () => {
    const selection = {
      shouldUse: false,
      selectedTemplates: [],
      reason: 'No template needed.',
    ...
    const agentExecutor = {
      executeLocalNoteSession: vi.fn().mockImplementation(async ({ onEvent }) => {
        await onEvent({
          ...
          runId: 'run_1',
          reason: 'User cancelled local note executor.',
        });

        return {
          status: 'failed',
          message: 'Executor returned after cancellation.',
        };
      }),
      executeProviderNativeSession: vi.fn(),
    };
    const agentSessionRepository = {
      create: vi.fn().mockResolvedValue({ id: 'agent_session_1' }),
      updateStatus: vi.fn().mockResolvedValue({ id: 'agent_session_1', status: 'cancelled' }),
    };
    const orchestrator = new RunOrchestrator(
      aiConfigService as never,
      textExecutor as never,
  ...
  it('settles provider-native sessions from terminal runtime events when present', async () => {
    const selection = {
      shouldUse: false,
      selectedTemplates: [],
      reason: 'No template needed.',
    ...
    const agentExecutor = {
      ...
      executeProviderNativeSession: vi.fn().mockImplementation(async ({ onEvent }) => {
        await onEvent({
          type: 'session.cancelled',
          runId: 'run_1',
          reason: 'User cancelled provider executor.',
        });
    ...
    const agentSessionRepository = {
      create: vi.fn().mockResolvedValue({ id: 'agent_session_native' }),
      updateStatus: vi.fn().mockResolvedValue({ id: 'agent_session_native', status: 'cancelled' }),
    };
    const orchestrator = new RunOrchestrator(
      aiConfigService as never,
      textExecutor as never,
    ...
    expect(runStepRepository.create).toHaveBeenCalledWith(expect.objectContaining({
      ...
      status: 'failed',
      title: 'Agent session 已取消',
      error: 'User cancelled provider executor.',
    }));
  });

  it('settles provider-native interrupted events before generic failed results', async () => {
    const selection = {
      shouldUse: false,
      selectedTemplates: [],
      reason: 'No template needed.',
    ...
    const agentExecutor = {
      ...
      executeProviderNativeSession: vi.fn().mockImplementation(async ({ onEvent }) => {
        await onEvent({
          type: 'session.interrupted',
          runId: 'run_1',
          reason: 'Provider executor lost liveness.',
        });

        return {
          status: 'failed',
          message: 'Provider executor returned a generic failure after interruption.',
        };
      }),
    };
    const agentSessionRepository = {
      create: vi.fn().mockResolvedValue({ id: 'agent_session_native' }),
      updateStatus: vi.fn().mockResolvedValue({ id: 'agent_session_native', status: 'failed' }),
    };
    const orchestrator = new RunOrchestrator(
      aiConfigService as never,
      textExecutor as never,
    ...
    expect(runStepRepository.create).toHaveBeenCalledWith(expect.objectContaining({
      ...
      status: 'failed',
      title: 'Agent session 已中断',
      error: 'Provider executor lost liveness.',
    }));
    expect(runStepRepository.create).not.toHaveBeenCalledWith(expect.objectContaining({
      title: 'Agent session 失败',
      error: 'Provider executor returned a generic failure after interruption.',
    }));
    expect(agentExecutor.executeLocalNoteSession).not.toHaveBeenCalled();
  });
