...
describe('AgentCliRunService', () => {
  ...
  it('projects Codex exec command_execution items into command progress', async () => {
      ...
      stdout: [
        ...
        JSON.stringify({
          ...
          item: {
            ...
            type: 'command_execution',
            command: '/bin/zsh -lc pwd',
            aggregated_output: '/Users/caoq/git/asterism\n',
            exit_code: 0,
            status: 'completed',
          },
        }),
    ...
    expect(runStepRepository.create).toHaveBeenCalledWith(expect.objectContaining({
      kind: 'tool_result',
      title: 'Codex CLI 命令执行：command_execution',
      output: expect.stringContaining('output=/Users/caoq/git/asterism'),
    }));
    expect(runStepRepository.create).toHaveBeenCalledWith(expect.objectContaining({
      kind: 'model',
      title: 'codex cli completed',
