...
import { getExposedAgentToolNames } from '../../shared/agent-tool-exposure.js';
...
function buildPrompt(
  ...
  if (input.type === 'draft' || input.type === 'agent') {
    if (input.type === 'agent') {
      const exposedTools = new Set(getExposedAgentToolNames({
        channel: 'text_prompt',
        policy: {
      ...
      const workspaceStepExample = exposedTools.has('workspace.search')
        ? [
            '    { "tool": "workspace.search", "input": { "query": "需要查找的关键词", "maxResults": 5 } },',
            '    { "tool": "workspace.read_file", "input": { "path": "相对工作区根目录的文件路径" } },',
          ]
        : [];
      const taskMutationStepExamples = exposedTools.has('task.update_next_step')
        ? [
        ...
        : [];
      const allowedToolParts = [...exposedTools];
      const workspaceGuidance = exposedTools.has('workspace.search')
        ? [
      ...
      const taskMutationGuidance = exposedTools.has('task.update_next_step')
        ? [
      ...
      return [
        ...
        '本次进入具体执行阶段，还必须读取并遵循以下 phase-loaded Agent 执行规范；只将其作为行为约束，不要把规范原文复述给用户：',
    ...
    return [
      ...
      '执行前必须读取并遵循以下 Taskplane 核心 Agent 上下文；用它判断任务节奏、执行边界和输出形态，但不要把规范原文复述给用户：',
      TASKPLANE_CORE_AGENT_CONTEXT,
      '本次进入具体输出阶段，还必须读取并遵循以下 phase-loaded Agent 执行规范；只将其作为行为约束，不要把规范原文复述给用户：',
      TASKPLANE_AGENT_PRINCIPLES,
      '输出要求：',
