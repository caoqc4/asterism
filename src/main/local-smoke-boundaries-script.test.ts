...
describe('local smoke script default boundaries', () => {
  ...
  it('keeps Agent API provider tool readiness smoke read-only and build-gated by default', () => {
    ...
    expect(result.output).toContain('providerNativeSessionRequirements=2/6');
    expect(result.output).toContain('providerNativeSessionMissingRequirements=provider_payload_identity,normalized_plan_identity,provider_call_ids,provider_web_search_calls');
    ...
    expect(result.output).toContain('providerNativeReadySessionRequirements=6/6');
  ...
  it('keeps runtime patch promotion routing readiness smoke read-only and build-gated by default', () => {
    ...
    expect(result.output).toContain('serviceEvidenceReadyOperatorApplySurface=service_explicit_apply');
