...
export function getGmailConnectorPreflight() {
  ...
  if (!accessToken) {
    issues.push('TASKPLANE_EXTERNAL_ACCESS_GMAIL_ACCESS_TOKEN is empty.');
  }
...
export function printGmailConnectorPreflight(result) {
  ...
  console.log('No Gmail request or task memory write was performed.');
  console.log('A later explicit Gmail source-ingestion action may call Gmail with the configured OAuth token.');
}
